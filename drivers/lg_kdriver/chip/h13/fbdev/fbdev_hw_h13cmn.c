...
void	FBDEV_H13CMN_WriteHdrRegs	( int fb_dev_id, FBDEV_H13_OSD_HDR_T* osd_hdr, FBDEV_H13_OSD_EXHDR_T* osd_exhdr )
{
	...
	{
		if ( g_fbdev_cfg->uhd_disp_mode && (fb_dev_id == LX_FBDEV_ID_OSD0 || fb_dev_id == LX_FBDEV_ID_OSD1) )
		{
			FBDEV_H13CMN_WriteUHDHdrRegs( fb_dev_id, osd_hdr ); /* use this function only for POSD0 (1) when UHD mode set */
		}
		else
		{
			FBDEV_H13CMN_WriteFHDHdrRegs( fb_dev_id, osd_hdr ); /* use this function when FHD mode set or cursor layer */
		}
	}

...
static LX_AUD_POWER_STATUS_T gADECPowerStatus_M14B0[LX_AUD_POWER_MAX] = {	\
...
*/
int M14B0_MIXED_IP_ADEC_PowerControl ( LX_AUD_POWER_BLOCK_T block, LX_AUD_POWER_MODE_T mode )
{
	return RET_OK;
...
//L9 A0 ADC uses AAD clock for operation, so AAD block must always enabled now.
	...
	{
		if( (gADECPowerStatus_M14B0[LX_AUD_POWER_AAD].block == block)
		  &&(gADECPowerStatus_M14B0[LX_AUD_POWER_AAD].mode  == mode ) )
		{
			//update power block
			gADECPowerStatus_M14B0[LX_AUD_POWER_AAD].block = block;
	...
	{
		if( (gADECPowerStatus_M14B0[LX_AUD_POWER_ADC].block == block)
		  &&(gADECPowerStatus_M14B0[LX_AUD_POWER_ADC].mode  == mode ) )
		{
			//update power block
			gADECPowerStatus_M14B0[LX_AUD_POWER_ADC].block = block;
	...
	{
		if( (gADECPowerStatus_M14B0[LX_AUD_POWER_DAC].block == block)
		  &&(gADECPowerStatus_M14B0[LX_AUD_POWER_DAC].mode  == mode ) )
		{
			//update power block
			gADECPowerStatus_M14B0[LX_AUD_POWER_DAC].block = block;
	...
	{
		if( (gADECPowerStatus_M14B0[LX_AUD_POWER_MIC].block == block)
		  &&(gADECPowerStatus_M14B0[LX_AUD_POWER_MIC].mode  == mode ) )
		{
			//update power block
			gADECPowerStatus_M14B0[LX_AUD_POWER_MIC].block = block;
	...
	AUD_KDRV_MIP_DEBUG("M14B0_MIXED_IP_ADEC_L9_PowerControl : block = %d, mode = %d\n", block, mode);
	return RET_OK;
#endif

...
int HDMI_M14A0_GetTimingInfo(LX_HDMI_TIMING_INFO_T *info)
{
	...
	//	_HDMI_M14A0_GetExtendTimingInfo(&bufTiming);		// get extend information
		...
		tmdsClock = _gM14A0HDMIPhyInform.tmds_clock; 	// XXX.XX KHz
#else
...
//	UINT8 aksv_data[5];
	...
	LINK_M14_Rd01(system_control_00, reg_hdmi_mode_sel, _gM14A0HDMIPhyInform.hdmi_mode);
	...
	if (access_phy[0] || access_phy[1] ||access_phy[2] ||access_phy[3] )
	{
		...
		_gM14A0HDMIPhyInform.tmds_clock = ((up_freq << 8) + down_freq); 	// XXX.XX KHz
	}
	else
		_gM14A0HDMIPhyInform.tmds_clock = 0;
...
#ifdef M14_THREAD_READ_PHY_REG_VALUE
	ui64TmdsClock = (UINT64)(_gM14A0HDMIPhyInform.tmds_clock ) * 10000;
#else
	PHY_REG_M14A0_RdFL(tmds_freq_1);
	PHY_REG_M14A0_RdFL(tmds_freq_2);
...
int HDMI_M14A0_Get_Driver_Status(LX_HDMI_DRIVER_STATUS_T *spHDMI_Driver_Status)
{
	...
	spHDMI_Driver_Status->TimingReadCnt = _gM14A0TimingReadCnt;
	spHDMI_Driver_Status->HDMIState = _gM14A0HDMIState;
	spHDMI_Driver_Status->PrevPixelEncoding = _gM14A0PrevPixelEncoding;
	memcpy(&spHDMI_Driver_Status->PrevAVIPacket, &_gM14A0PrevAVIPacket, sizeof(LX_HDMI_INFO_PACKET_T));
	memcpy(&spHDMI_Driver_Status->PrevVSIPacket, &_gM14A0PrevVSIPacket, sizeof(LX_HDMI_INFO_PACKET_T));
	spHDMI_Driver_Status->AVIReadState = _gM14A0AVIReadState;
	spHDMI_Driver_Status->VSIState = _gM14A0VSIState;
	spHDMI_Driver_Status->AVIChangeState = _gM14A0AVIChangeState;
	spHDMI_Driver_Status->PortSelected = 0;
	spHDMI_Driver_Status->MHLContentOff = 0;

	spHDMI_Driver_Status->AudioMuteState = _gM14A0AudioMuteState;
	spHDMI_Driver_Status->AudioArcStatus = _gM14A0AudioArcStatus;
	spHDMI_Driver_Status->IntrAudioState = _gM14A0IntrAudioState;
	spHDMI_Driver_Status->HdmiPortStableCount = _gM14A0HdmiPortStableCount;
	spHDMI_Driver_Status->HdmiFreqErrorCount = _gM14A0HdmiFreqErrorCount;
	spHDMI_Driver_Status->HdmiAudioThreadCount = _gM14A0HdmiAudioThreadCount;
	spHDMI_Driver_Status->IntrBurstInfoCount = _gM14A0IntrBurstInfoCount;
	spHDMI_Driver_Status->TaskBurstInfoCount = _gM14A0TaskBurstInfoCount;
	memcpy(&spHDMI_Driver_Status->HdmiAudioInfo, &_gM14A0HdmiAudioInfo, sizeof(LX_HDMI_AUDIO_INFO_T));

	spHDMI_Driver_Status->Intr_vf_stable = _gM14A0Intr_vf_stable;
	spHDMI_Driver_Status->Intr_avi = _gM14A0Intr_avi;
	spHDMI_Driver_Status->Intr_src_mute = _gM14A0Intr_src_mute;
	spHDMI_Driver_Status->Intr_vsi = _gM14A0Intr_vsi;
	...
	spHDMI_Driver_Status->CHG_AVI_count_MHL = 0;
	spHDMI_Driver_Status->CHG_AVI_count_EQ = 0;
	spHDMI_Driver_Status->TMDS_ERROR_EQ = 0;

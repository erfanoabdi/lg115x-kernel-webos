...
#include "ademod_common_UserSettings.c"

extern UINT8 RMM_COUNTRY_GROUP_LOCATION;
extern UINT8 SVC_CTRL_LGT10ColorMode;
...
LX_ADEMOD_Result ADEMOD_Preset_Set(Handle_t hDevice, LX_ADEMOD_SmartTuneDescriptor *pSmartTune, UINT32 Section)
{
		...
		if (Section == LX_ADEMOD_PRESET_SYSTEM_INIT) { // System init
			...
			if (pSmartTune->Filter & LX_ADEMOD_SmartTuneFilterAnalog) { // Analog mode
			
				if(lx_chip_rev() <= LX_CHIP_REV( M14, A1))
				{
					switch (pSmartTune->Filter & LX_ADEMOD_SmartTuneFilterMode) // Video Standard
					{
...
#else
				if (pSmartTune->Filter & LX_ADEMOD_SmartTuneFilterCable) { // Cable input
					switch (pSmartTune->Filter & LX_ADEMOD_SmartTuneFilterMode) // Video Standard
					{

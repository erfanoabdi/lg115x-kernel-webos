...
#if 0
	badstatictrackfdeq_3_0 = 0x7;
	EQ_V_RdFL(eq_v_ctrl51); // regAddr = 0x2133[7:4]
	EQ_V_Wr01(eq_v_ctrl51,badstatictrackfdeq_3_0, badstatictrackfdeq_3_0);
	EQ_V_WrFL(eq_v_ctrl51);

	badphderotbwsel_3_0 = 0x8;
	baddynphderotbwsel_3_0 = 0x8;
	EQ_V_RdFL(eq_v_ctrl58); // regAddr = 0x213a[7:4] , 0x213a[3:0]
	EQ_V_Wr02(eq_v_ctrl58,badphderotbwsel_3_0,badphderotbwsel_3_0, baddynphderotbwsel_3_0, baddynphderotbwsel_3_0);
...
//	DEMOD_M14B0_Get_Packet_Error(&packeterror); //jeongpil.yun--> packet error  should be read at once within one second !! this line conflicts with other functions.
	...
	if( (maxpwrmain>0x60) )
	{
		...
		if(snrref_5_0>0x1d)
		{
			...
			samgsautoedl_v_1_0 = 0x3;
			Sync_RdFL(sync_ctrl60); // regAddr = 0x103c[3:0]
			Sync_Wr01(sync_ctrl60, samgsautoedl_v_1_0, samgsautoedl_v_1_0);
			Sync_WrFL(sync_ctrl60);
			DEMOD_TRACE(" ###chungiiiiii0226 snrref_5_0 [0x%x]\n",snrref_5_0);
		}
		else if(snrref_5_0>0x13)
		{
			samgearshen = 0x1;
			Common_RdFL(common_ctrl61); // regAddr = 0x003d[6]
			Common_Wr01(common_ctrl61, samgearshen, samgearshen);
...
//	DEMOD_M14B0_Get_Packet_Error(&packeterror); //jeongpil.yun--> packet error  should be read at once within one second !! this line conflicts with other functions.
	...
	else
	{
		if(tperrcount25ms>0)
		{
			DEMOD_TRACE(" ###chungiii DEBUG ## tperrcount25ms>0  _gM14B0GBB_Phn_state=0\n");
			_gM14B0GBB_Phn_state = 0;
		}
		else
		{
			DEMOD_TRACE(" ###chungiii DEBUG ## tperrcount25ms<0  _gM14B0GBB_Phn_state=1\n");
			_gM14B0GBB_Phn_state = 1;
		}
					...
					if (LX_DEMOD_TUNE_NORMAL == pDemodInfo->setCfgParam.vsb.tuneMode)
					{
						...
						else
						{
							...
							else
							{
								...
								if( ((packetError < LX_DEMOD_PACKET_ERROR_THD_100) && (Cochannel == FALSE)) && (_gM14B0GBB_Tpblockon == FALSE) )
								{
									//for debug chungiii
									//OS_MsecSleep(200);
...
int DEMOD_M14B0_QAM_Set_DefaultRegisterValue(void)
{
	...
	Sync_RdFL(sync_ctrl54); // regAddr = 0x103c[7:4]

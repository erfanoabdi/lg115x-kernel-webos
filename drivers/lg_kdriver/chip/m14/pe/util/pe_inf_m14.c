...
#define PE_INF_M14_DRV_VER		"2014/05/19/10:00"
...
int PE_INF_M14_SetDisplayInfo(LX_PE_INF_DISPLAY_T *pstParams)
{
	...
	do{
		...
		PE_INF_M14_DBG_PRINT("set[%d] : src:%d,res:%d (h:%d,v:%d)\n"\
			...
			"in_win:%d,%d,%d,%d,out_win:%d,%d,%d,%d\n"\
			...
			pp->out_win.h_size, pp->out_win.v_size, \
		...
		if(PE_KDRV_VER_M14)
		{
			...
			if(_g_pe_inf_m14_trace)
			{
				if(count>PE_PRINT_COUNT_NUM)
				{
					...
					PE_INF_M14_DBG_PRINT("disp[0] : in_win:%d,%d,%d,%d,out_win:%d,%d,%d,%d\n", \
					...
					PE_INF_M14_DBG_PRINT("disp[1] : in_win:%d,%d,%d,%d,out_win:%d,%d,%d,%d\n", \
		...
		if(PE_KDRV_VER_M14)
		{
			...
			switch(rd_cnt)
			{
				...
				PE_INF_M14_CASE_SPRINT(14,rd_cnt++,buffer,\
					"[%02d]disp0: in_win:%d,%d,%d,%d,out_win:%d,%d,%d,%d\n",rd_cnt,\
					...
					disp0_inf->out_win.h_size,disp0_inf->out_win.v_size);
				...
				PE_INF_M14_CASE_SPRINT(19,rd_cnt++,buffer,\
					"[%02d]disp1: in_win:%d,%d,%d,%d,out_win:%d,%d,%d,%d\n",rd_cnt,\
					...
					disp1_inf->out_win.h_size,disp1_inf->out_win.v_size);
						...
						switch (fn_t)
						{
							case PE_ITEM_PKTFUNC(LX_PE_INF_DISPLAY_T):
								PE_INF_M14_CPY_PARAM_TO_STR(LX_PE_INF_DISPLAY_T, \
									...
									p.in_v_size, p.mode.is_ext_frc, p.mode.is_oled);
								break;
							default:
								break;
						}

...
static int PE_TSK_M14_PicTask(void *data)
{
	...
	while(1)
	{
		...
		if (pCfg->hdlr_opr.is_registered == TRUE)
		{
			if ((hdlr_cnt == pCfg->hdlr_opr.count) && \
				((task_cnt&PE_TSK_M14_CHK_INC_HDL_CNT_NUM)==PE_TSK_M14_CHK_INC_HDL_CNT_NUM))
			{
				PE_PRINT_NOTI("pic_sync_handler count(%d) is not increased.\n", \
					hdlr_cnt);
			}
			if(pOpr->dbgmode==PE_TSK_M14_DBG_ON)
			{
				PE_TSK_M14_PRINT("[pic_task]hdlr_opr.count : %d -> %d\n", \
		...
		if (pOpr->pause_task == PE_TSK_M14_RUN_STOP_TASK)
		{
			...
			interruptible_sleep_on(&_wq_pe_m14);
		}
	}
	return RET_OK;
}

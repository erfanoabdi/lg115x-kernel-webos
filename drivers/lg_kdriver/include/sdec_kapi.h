...
// Memory Pool
#define SDEC_IOW_MM_Init					_IOWR(SDEC_IOC_MAGIC, 31, LX_SDEC_GPB_INFO_T)
#define SDEC_IOW_MM_Final					_IOWR(SDEC_IOC_MAGIC, 32, UINT32)
#define SDEC_IOW_MM_Free					_IOWR(SDEC_IOC_MAGIC, 33, UINT32)
#define SDEC_IOR_MM_Alloc					_IOWR(SDEC_IOC_MAGIC, 34, LX_SDEC_MM_Alloc)
#define SDEC_IOR_MM_GetStat					_IOWR(SDEC_IOC_MAGIC, 35, LX_SDEC_MM_GetStat)
...
typedef enum
{
	...
	LX_SDEC_INPUT_DVR				= 6,			/**< Input From DVR UP Channel */
	LX_SDEC_INPUT_BDRC				= 7,			/**< Input From BDRC from GPB */
} LX_SDEC_INPUT_T;
...
/**
 * SDEC Input Mode setting.
 * detailed description.
 * @see LX_SDEC_SEL_INPUT_T
 */
typedef enum
{
	LX_SDEC_INPUTMODE_SERIAL0		= 0x0,			/**< Serial from External DEMOD or CI */
	LX_SDEC_INPUTMODE_SERIAL1		= 0x1,			/**< Serial from External DEMOD or CI */
	LX_SDEC_INPUTMODE_PARALLEL0		= 0x2,			/**< Parallel from internal DEMOD or CI */
	LX_SDEC_INPUTMODE_PARALLEL1		= 0x3,			/**< Parallel from internal DEMOD or CI */
	LX_SDEC_INPUTMODE_SERIAL2		= 0x4,			/**< Serial from Internal But it's for JAPAN */
	LX_SDEC_INPUTMODE_SERIAL3		= 0x5,			/**< Serial from External But it's for JAPAN */
} LX_SDEC_INPUTMODE_T;

/**
 * SDEC Input Port
 * detailed description.
 * @see LX_SDEC_CFG_INPUT_T
 */
typedef enum
{
	LX_SDEC_INPUTPORT_SERIAL0		= 0x0,			/**< Serial from External DEMOD or CI */
	LX_SDEC_INPUTPORT_SERIAL1		= 0x1,			/**< Serial from External DEMOD or CI */
	LX_SDEC_INPUTPORT_PARALLEL0		= 0x2,			/**< Parallel from internal DEMOD or CI */
	LX_SDEC_INPUTPORT_PARALLEL1		= 0x3,			/**< Parallel from internal DEMOD or CI */
} LX_SDEC_INPUTPORT_T;
...
typedef struct
{
	UINT32	others	: 24;			/**< Reserved Types for Future */
	UINT32	rawts	: 1;			/**< RAW TS */
	UINT32	gpb		: 1;			/**< Section/PES */
	UINT32	pcr		: 1;			/**< PCR */
	UINT32	dvr 	: 1; 			/**< DVR */
	UINT32	adec1 	: 1;			/**< ADEC1*/
	UINT32 	adec0 	: 1; 			/**< ADEC0*/
	UINT32 	vdec1 	: 1;			/**< VDEC1*/
	UINT32	vdec0 	: 1;			/**< VDEC0 */
} LX_SDEC_DEST_T;
...
typedef struct
{
	...
	UINT32						/* CDIP & CDIOP */
								...
								sync_type	: 2,		/**< in  -	Synchronization type ( only for input ) */
								/* CDIOP Only */
								val_sel	: 1,		/**< in  -	VAL pin select */
...
typedef struct
{
	UINT32				chip_rev;						/**< out -  overall chip revision. use LX_CHIP_REV() macro. */
...
/**
 * Information of SDEC gpb
 * @see SDEC_IOR_MM_Alloc
 */
typedef struct
{
	UINT32						uiGpbSize;				/**< in  	-  Size of GPB */
	UINT32						uiGpbAddr;				/**< out	-  returned address which is allcated */
} LX_SDEC_MM_Alloc;

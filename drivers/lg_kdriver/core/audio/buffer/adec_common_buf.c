	...
	ADEC_COMMON_BUF_STATUS_T* _status)
{
	...
	if(auSize < (long long)(_buf->maxSize)) {
		ADEC_BUF_ASSERT((_buf->indexTable[index].bufOffset <= _status->readOffset &&
			(long long)(_buf->indexTable[index].bufOffset) + _buf->indexTable[index].base.size >= (long long)(_status->readOffset)) ||
			(_buf->indexTable[index].bufOffset <= _status->readOffset + _buf->maxSize &&
			(long long)(_buf->indexTable[index].bufOffset) + _buf->indexTable[index].base.size >= (long long)(_status->readOffset) + (long long)(_buf->maxSize)),
			_buf, _status, ADEC_ERR_DATA_CORRUPTED,
			"Index Table Broken.\n");

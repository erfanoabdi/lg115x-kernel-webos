...
{
	AUD_KDRV_DEBUG("[%s] AUD_RunSuspend : Start...\n", AUDIO_MODULE);
	...
	AUD_KDRV_PRINT("ADEC_RunSuspend : Done!\n");
	return 0;
}
...
int AUD_RunResume(void)
{
	...
	//Restart audio master device after DSP H/W Reset
...
int AUDIO_Init(void)
{
	...
	OS_DEBUG_DisableModuleByIndex(g_AudDebugFd, LX_LOGM_LEVEL_INFO);						//AUD_KDRV_INFO
	...
	//OS_DEBUG_EnableModuleByIndex(g_AudDebugFd, LX_LOGM_LEVEL_TRACE_ALSA_PRINT, DBG_COLOR_RED );	//AUD_KDRV_ALSA_PRINT
	...
	else
	{
		...
		{
			AUD_KDRV_DEBUG("[%s] platform register done\n", AUDIO_MODULE);
		}
	}
#endif
	...
	{
		AUD_KDRV_DEBUG_TMP ("LX_CHIP_REV(H14, XX)(0x%X)\n", lx_chip_rev());
	...
	{
		DBG_PRINT_ERROR("AUD : LX_CHIP_REV => Unknown(0x%X) : ERROR\n", lx_chip_rev());
		return -EIO;
	}

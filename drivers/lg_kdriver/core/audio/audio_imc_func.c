...
SINT32 AUDIO_IMC_SendCmd(int cmd, ADEC_MODULE_ID targetModule)
{
	...
	switch (cmd)
	{
		case ADEC_CMD_INIT:
			...
			pModInfo->status = AUD_MOD_STATUS_INIT;
			break;

		case ADEC_CMD_START:
			imcCmdParam->callback	= _IMC_ResponseStartCB;
			pModInfo->status = AUD_MOD_STATUS_START;
			break;

		case ADEC_CMD_STOP:
			imcCmdParam->callback	= _IMC_ResponseStopCB;
			pModInfo->status = AUD_MOD_STATUS_STOP;
			break;

		case ADEC_CMD_FLUSH:
			imcCmdParam->callback	= _IMC_ResponseFlushCB;
			pModInfo->status = AUD_MOD_STATUS_FLUSH;
			break;

...
UINT8   CIS_ParseTuple_count = 0;

/*=============================================================================+
| Local Function Prototype                                                     |
+=============================================================================*/

static int CIS_ReadTuple(UINT32 baseAddr, UINT8 *pBuf, UINT32 *pOffset, SINT16 nSleepTime);
...
SINT32 CIS_Config(UINT8 mId)
{
	...
	UINT32			baseAddr = CiModBaseAddr[MOD_A];
	int				chkCnt = 0;
	UINT32			offset = 0;
	int				rdRet;
	...
	CI_DTV_SOC_Message(CIS_INFO,"Configuring CIS of module %c \n", 'A'+mId); 

	pStatus = &CIS_infoTupleStatus[mId];

	CI_DTV_SOC_Message(CIS_INFO,"baseAddr: 0x%x \n", baseAddr);
	...
	while( chkCnt-- )
	{
		...
		if( BYTE(baseAddr) == CISTPL_DEVICE_OA )
		{
			if( (MAX_VALID_TUPLE_CNT_CHK - 1) == chkCnt )
			{
#ifdef _CI_KDRV_DELAY_USLEEP_RANGE_
				usleep_range(gCI_KDRV_Delay[CI_D_CIS_CONFIG_FIRST_TUPLE_OK]*1000, gCI_KDRV_Delay[CI_D_CIS_CONFIG_FIRST_TUPLE_OK]*1000);
#else
				OS_MsecSleep( gCI_KDRV_Delay[CI_D_CIS_CONFIG_FIRST_TUPLE_OK] );
#endif
			}
...
#ifdef _CI_KDRV_DELAY_USLEEP_RANGE_
		usleep_range(gCI_KDRV_Delay[CI_D_CIS_CONFIG_FIRST_TUPLE_NG]*1000, gCI_KDRV_Delay[CI_D_CIS_CONFIG_FIRST_TUPLE_NG]*1000);
#else
		OS_MsecSleep( gCI_KDRV_Delay[CI_D_CIS_CONFIG_FIRST_TUPLE_NG] );
#endif
	}

	if( chkCnt <= 0 )
	{
		CI_DTV_SOC_Message(CIS_ERR,"CIS_Config: the first tuple is invalid ");
		goto CIS_CONFIG_FAIL;
	}
...
CIS_READ_ANALYZE_STEP:
	...
	while( chkCnt-- )
	{
		...
		{
#ifdef _CI_ACCESS_BURST_MODE_
			rdRet = CIS_ReadTuple32(baseAddr, aui8Buf, &offset, nSleepTime); 
#else
			rdRet = CIS_ReadTuple(baseAddr, aui8Buf, &offset, nSleepTime);
#endif
		}
		else
		{
		...
		CI_DTV_SOC_Message(CIS_TUPLE_DUMP,"Tuple Code=0x%02X, Tuple Length=%3d -------------------------------\n",
																		aui8Buf[0], aui8Buf[1]+2);
		CI_DTV_SOC_Message(CIS_TUPLE_DUMP,"-----------------------------------------------------------------\n");

		if( CIS_ParseTuple(mId, aui8Buf, pStatus) == CIS_PARSETIMEOUT )
		{
			CI_DTV_SOC_Message(CIS_ERR,"CIS configuration FAILED #1\n");
			break;
		}
		if( rdRet == TUPLE_LAST )
			break;
	...
	{	// if the abnormal case, the try again (max 3, delay time is changed)
		...
		{
			CI_DTV_SOC_Message(CIS_ERR,"Again get CIS configuration (%d try)\n", nNumofTry_ReadTuple);
		...
		{
			CI_DTV_SOC_Message(CIS_ERR,"CIS configuration FAILED #2\n" );
		}
	}
...
#ifdef _CI_KDRV_DELAY_USLEEP_RANGE_
	usleep_range(gCI_KDRV_Delay[CI_D_CIS_END_WRITE_COR]*1000, gCI_KDRV_Delay[CI_D_CIS_END_WRITE_COR]*1000);
#else
	OS_MsecSleep( gCI_KDRV_Delay[CI_D_CIS_END_WRITE_COR] );
#endif
...
static int CIS_ReadTuple32(UINT32 baseAddr, UINT8 *pBuf, UINT32 *pOffset, SINT16 nSleepTime)
{
	...
	{
		CI_DTV_SOC_Message(CIS_INFO,"tuple start address is not aligned by 4 [0x%x]",baseAddr+offset);
...
#ifdef _CI_KDRV_DELAY_USLEEP_RANGE_
		usleep_range(gCI_KDRV_Delay[CI_D_CIS_DURING_READ_TUPLE]*1000, gCI_KDRV_Delay[CI_D_CIS_DURING_READ_TUPLE]*1000);
#else
		OS_MsecSleep( gCI_KDRV_Delay[CI_D_CIS_DURING_READ_TUPLE] );
#endif
	}
	else
	{
	...
	while(tuple_length_q--)
	{
		...
		*pData++ = (UINT8)(tuple_value & 0x000000FF); 			/* save 1st byte */
		*pData++ = (UINT8)((tuple_value & 0x00FF0000) >> 16); 	/* save 2nd byte */
	...
	if(tuple_length_r == 1)
	{
		CI_DTV_SOC_Message(CIS_INFO,"tuple 1 byte remained [0x%x]",baseAddr+offset);
		...
		tuple_value_word = WORD(baseAddr+offset);
		*pData++ = (UINT8)(tuple_value_word & 0x00FF);
	...
	{
		/* ERROR */
		CI_DTV_SOC_Message(CI_ERR,"tuple length remainder is not valid [%d]",tuple_length_r);
	}
	...
	*pOffset = offset;
	...
	usleep_range(gCI_KDRV_Delay[CI_D_CIS_DURING_READ_TUPLE]*1000, gCI_KDRV_Delay[CI_D_CIS_DURING_READ_TUPLE]*1000);
#else
	OS_MsecSleep( nSleepTime );		// apply flexible delay time
#endif
...
#ifdef _CI_KDRV_DELAY_USLEEP_RANGE_
	usleep_range(nSleepTime*1000, nSleepTime*1000);
#else
	OS_MsecSleep( nSleepTime );		// apply flexible delay time
#endif
...
static int CIS_ParseTuple(MOD_ID mId, UINT8 *pBuf, CISTPL_STATUS *pStatus)
{
	...
	{
	case CISTPL_VERS_1 :
		CI_DTV_SOC_Message(CIS_INFO,"[CISTPL_VERS_1] =================================================\n" );
		pStatus->vers_1 = CIS_ParseVers1(mId, pBuf);
		break;

	case CISTPL_CONFIG :
		CI_DTV_SOC_Message(CIS_INFO,"[CISTPL_CONFIG] =================================================\n" );
		pStatus->config = CIS_ParseConfig(mId, pBuf);
		break;

	case CISTPL_CFTABLE_ENTRY :
		CI_DTV_SOC_Message(CIS_INFO, "[CISTPL_CFTABLE_ENTRY] ==========================================\n" );
	...
	case CISTPL_MANFID :
		CI_DTV_SOC_Message(CIS_INFO,"[Tuple-0x%x] Skipped ============================================\n", tupleCode );
		break;
	...
	default :
		...
		if(CIS_ParseTuple_count >= MAX_TUPLE_COUNT_ERR)
	   	{
		     CI_DTV_SOC_Message(CIS_ERR,"^e^========@ CIS_ParseTuple error =======\n" );

		     return CIS_PARSETIMEOUT;
		}

#ifdef _CI_KDRV_DELAY_USLEEP_RANGE_
		usleep_range(gCI_KDRV_Delay[CI_D_CIS_PARSE_NON_CI_TUPLE]*1000, gCI_KDRV_Delay[CI_D_CIS_PARSE_NON_CI_TUPLE]*1000);
#else
		OS_MsecSleep( gCI_KDRV_Delay[CI_D_CIS_PARSE_NON_CI_TUPLE] );
#endif
		break;
	}

	CI_DTV_SOC_Message(CIS_PARSE_DUMP,"=================================================================\n" );
...
static TPL_STATUS CIS_ParseVers1(MOD_ID	mId, UINT8 *pBuf)
{
	...
	{
		CI_DTV_SOC_Message(CIS_PARSE_DUMP,"CIS - Actual Tuple Len - %d, Source buf(Tuple) size - %d\n",
	...
	if( (pVers1->major==TPLLV1_MAJOR) && (pVers1->minor==TPLLV1_MINOR) )
		ret = DVB_CI_COMPLIANT;
	else
		ret = DVB_CI_NOT_COMPLIANT;

	CI_DTV_SOC_Message(CIS_PARSE_DUMP,"  Major Version: 0x%02X\n", pVers1->major );
	CI_DTV_SOC_Message(CIS_PARSE_DUMP,"  Minor Version: 0x%02X\n", pVers1->minor );
	...
	while( ( restLen-- ) && ( strCnt < 4 ) )
	{
		...
		else
		{
			...
			*(pStr[strCnt]) = '\0';
			...
			if( 2 == strCnt || 3 == strCnt )		// add1 or add2
			{
...
//					pVers1->ui8IsItCIPlus = CIS_IsItCIPlusCAM( pData, restLen );
	...
	CI_DTV_SOC_Message(CIS_PARSE_DUMP,"  Manufact Str : %s\n", pVers1->manufact );
	CI_DTV_SOC_Message(CIS_PARSE_DUMP,"  Product Str  : %s\n", pVers1->product );
	CI_DTV_SOC_Message(CIS_PARSE_DUMP,"  Add1 Str	  : %s\n", pVers1->add1 );
	CI_DTV_SOC_Message(CIS_PARSE_DUMP,"  Add2 Str	  : %s\n", pVers1->add2 );
	CI_DTV_SOC_Message(CIS_PARSE_DUMP,"  Is it CI+ CAM: %d\n", pVers1->ui8IsItCIPlus );
...
static TPL_STATUS CIS_ParseConfig(MOD_ID mId, UINT8 *pBuf)
{
	...
	CI_DTV_SOC_Message(CIS_PARSE_DUMP,"  TPCC_RASZ : %02d bytes\n", tpccRasz );
	CI_DTV_SOC_Message(CIS_PARSE_DUMP,"  TPCC_RMSZ : %02d bytes\n", tpccRmsz );
	CI_DTV_SOC_Message(CIS_PARSE_DUMP,"  TPCC_RFSZ : %02d bytes\n", tpccRfsz );

	if( tupleLen < (tpccRasz+tpccRmsz+tpccRfsz+2) )
	{
		CI_DTV_SOC_Message(CIS_ERR,"CIS_ParseConfig: size error \n" );
	...
	CI_DTV_SOC_Message(CIS_PARSE_DUMP,"  Last Index Entry    : 0x%02X\n", pConfig->lastIndex );
	...
	CI_DTV_SOC_Message(CIS_PARSE_DUMP,"	Config Reg Base Addr: 0x%08X\n",	pConfig->baseAddr );
	...
	if( tupleBody & 0x40 )
	{
		...
		CI_DTV_SOC_Message(CIS_PARSE_DUMP, "  Interface Num       : 0x%04X\n", stciIfn );
		...
		CI_DTV_SOC_Message(CIS_PARSE_DUMP, "  String Info         : %s\n", pCcstCif->str );
	}
	else
	{
		return DVB_CI_NOT_COMPLIANT;
...
static TPL_STATUS CIS_ParseCftableEntry(MOD_ID mId, UINT8 *pBuf)
{
	...
	if( tupleBody == STCE_EV ) {
		...
		CI_DTV_SOC_Message(CIS_INFO,"	Environment String    : %s\n", pStceEvpd->environment );
	}
	else
	{
			pNext = pData;
	...
	if( tupleBody == STCE_PD )
	{
		...
		CI_DTV_SOC_Message(CIS_INFO,"  Physical Device String: %s\n", pStceEvpd->physicalDev );
	}
...
static TPL_STATUS CIS_ParseManfid(MOD_ID mId, UINT8 *pBuf)
{
	...
	CIS_PARSE_DUMP( "	Manufacturer ID         : 0x%04X\n", pManfid->manf );
	CIS_PARSE_DUMP( "	Card ID & Revision Info : 0x%04X\n", pManfid->card );
...
//					CI_DTV_SOC_Message(CIS_CIPLUS,"[%c]",*pui8Tmp);
				...
				cCompStr[i]='\0';
//				CI_DTV_SOC_Message(CIS_CIPLUS,"\n");
				CI_DTV_SOC_Message(CIS_CIPLUS,"[CI-CISTPL_VERS1] string :: %s\n",cCompStr);
...
//					CI_DTV_SOC_Message(CIS_CIPLUS,"string 2 :: %s\n",pStr);
					...
					if( (pStr[0] == '+') || (pStr[0] == '-') || (pStr[0] == '*') ) pStr = pStr + 1;
					...
					CI_DTV_SOC_Message(CIS_CIPLUS,"[CI-CISTPL_VERS1] ciplus version = %d\n",ui32Number);
...
//					CI_DTV_SOC_Message(CIS_CIPLUS,"string prof :: %s\n",pStr);
					...
					if(!strncasecmp( cCompStr, "ciprof=0x", 9))
					{
						...
						CI_DTV_SOC_Message(CIS_CIPLUS,"[CI-CISTPL_VERS1] ciprof number = 0x%x\n",ui32Number);
//						CI_DTV_SOC_Message(CIS_CIPLUS,"[CI-CISTPL_VERS1] ciprof number = %d\n",ui32Number);
				...
				if( (*pui8Tmp == '\0') || (*pui8Tmp == ']') || (pui8Tmp > (i_pui8Buf + i_ui32Len)) )
				{
					return iRet;
				}
#endif

...
typedef struct
{
	...
	// variable to set aad module.
 	LX_AAD_SIF_SOUNDSYSTEM_T	eSystem;
 	LX_AAD_SIF_STANDARD_T		eStandard;
}AAD_DRVDATA_T;
#endif
...
extern LX_AAD_INIT_TYPE_T 		g_eAadInitType;
extern LX_AAD_HIGH_DEV_SET_T 	g_eHighDevMode;
extern LX_AAD_SIF_THRESHOLD_T 	g_stThresholdLevel;
extern LX_AAD_SIF_SOUNDSYSTEM_T	g_i_eSystem;
extern LX_AAD_SIF_STANDARD_T	g_i_eStandard;
extern LX_ATV_AUDIO_MODE_SET_T 	g_i_eAudioModeSet;
#endif
...
static int AAD_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	/* Save a AAD local data. */
 	drv_data->eSystem	= g_i_eSystem;
 	drv_data->eStandard	= g_i_eStandard;
...
static int AAD_resume(struct platform_device *pdev)
{
	...
	/* Set a default AAD local data. */
 	g_i_eSystem   = LX_AAD_SIF_SYSTEM_UNKNOWN;
 	g_i_eStandard = LX_AAD_SIF_NUM_SOUND_STD;

	/* Resume operation for AAD Module. */
	(void)AAD_SIF_InitModule(g_eAadInitType, TRUE);
	(void)AAD_SIF_SetHighDeviationMode(g_eHighDevMode);
	(void)AAD_SIF_SetThresholdLevel(g_stThresholdLevel);
	(void)AAD_SIF_SetBandSetup(drv_data->eSystem);
	(void)AAD_SIF_SetModeSetup(drv_data->eStandard);
	(void)AAD_SIF_SetUserAnalogMode(g_i_eAudioModeSet);
...
int AAD_Init(void)
{
	...
	OS_DEBUG_EnableModuleByIndex (g_aad_debug_fd, LX_LOGM_LEVEL_ERROR, DBG_COLOR_NONE);		//AAD_ERROR
	OS_DEBUG_DisableModuleByIndex(g_aad_debug_fd, LX_LOGM_LEVEL_WARNING);					//AAD_WARN
	...
	if(gpRealRegAAD == NULL || gpRegAAD == NULL)
	{

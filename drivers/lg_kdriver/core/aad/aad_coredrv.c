...
----------------------------------------------------------------------------------------*/
//#define ENABLE_M14_BTSC_50KHZ_FILTER	//Enable M14 BTSC 50Khz Demod Filter Mode
...
static int AAD_SECAM_L_SetManualVolumeForAdjust_H13 ( void );
...
#ifdef ENABLE_M14_BTSC_50KHZ_FILTER
static int AAD_SetBTSCDemodFilter( void );
#endif
...
int AAD_SIF_InitModule ( LX_AAD_INIT_TYPE_T eInitType, BOOLEAN bInitialized )
{
	...
	//Read registers
	AAD_RdFL(audio_out_ctrl);
	AAD_DEBUG ("AAD Initialzed 0x%X(audio_output_ctrl = 0x%X)\n", eInitType, AAD_Rd(audio_out_ctrl));
	return RET_OK;
}
...
int AAD_SIF_SetHighDeviationMode ( LX_AAD_HIGH_DEV_SET_T eHighDevMode )
{
	...
	if (lx_chip() == LX_CHIP_M14)
	{
		//In IN(India) model, Do not mute on.
	...
	if (g_eAadInitType == LX_AAD_INIT_DVB_IN_SELECT && eHighDevMode.highDevLevel == LX_AAD_HIGH_DEV_LEVEL_OFF && lx_chip() == LX_CHIP_M14)
	{
		...
		AAD_Wr01(manual_out_lvl_left_ch,  levlft, 0x28); // HDEV option : Off
		AAD_Wr01(manual_out_lvl_right_ch, levrt,  0x28);
	...
	//In IN(India) model, Adjust Volume Level.
	if (g_eAadInitType == LX_AAD_INIT_DVB_IN_SELECT && eHighDevMode.highDevLevel == LX_AAD_HIGH_DEV_LEVEL_4 && lx_chip() == LX_CHIP_M14)
	{
		//Read manual_output_level_left_ch reg.
		AAD_RdFL(manual_out_lvl_left_ch);
...
int AAD_SIF_SetBandSetup ( LX_AAD_SIF_SOUNDSYSTEM_T eSoundSystem )
{
	...
	{
		//Set a BTSC Pre Expander Gain for M14/H14(H14, M14 BTSC Stereo Separation : improve +4~5db)
		/* Check LX Chip Revision Number */
		if (lx_chip_rev() >= LX_CHIP_REV(H14, A0))
		{
		...
		switch ( eSoundSystem )
		{
			...
			case LX_AAD_SIF_SYSTEM_MN:
				if ( g_eAadInitType == LX_AAD_INIT_ATSC_BTSC_SELECT || g_eAadInitType == LX_AAD_INIT_ATSC_BTSC_US_SELECT )
				{
					...
					#ifdef ENABLE_M14_BTSC_50KHZ_FILTER
					/* Set a BTSC Demod Filter to increase SNR in QA Measurement. */
					(void)AAD_SetBTSCDemodFilter();
					#endif	//#ifdef ENABLE_M14_BTSC_50KHZ_FILTER
				}
				else
				{
...
int AAD_SIF_GetBandDetect ( LX_AAD_SIF_SYSTEM_INFO_T *pstSystemInfo )
{
	...
	if (iRetVal == RET_OK)
	{
		...
		//In ID(Indonesia) model, modify carrier1 quality value for weak signal.
		if ( g_eAadInitType == LX_AAD_INIT_DVB_ID_SELECT )
		{
			...
			if (ui16Carrier1Quality < CAR1_QUAL_THD_WEAK_SIGNAL_ID && g_ui16Carrier1Quality_ID < (CAR1_QUAL_THD_WEAK_SIGNAL_ID + 1))
			{
				...
				if (ui16Carrier1Quality < CAR1_QUAL_THD_WEAK_SIGNAL_ID && ui16DiffCarrier1Quality >= CAR1_QUAL_DIFF_WEAK_SIGNAL_ID)
				{
					...
					AAD_ERROR("Forced Carrier1 Quality 0x%X, Diff 0x%X\n", ui16Carrier1Quality, ui16DiffCarrier1Quality);
				}
				else
				{
					...
					AAD_ERROR("Carrier1 Quality 0x%X, Diff 0x%X\n", ui16Carrier1Quality, ui16DiffCarrier1Quality);
				}
			}
			else
			{
				AAD_ERROR("Carrier1 Quality 0x%X\n", ui16Carrier1Quality);
			}
...
#else	//For band detect test only
		for( i = 0; i < 10; i++ )
		{
			AAD_RdFL(car1_fm_qual0);
			AAD_Rd01(car1_fm_qual0, cr1ql, value);
...
int AAD_SIF_GetCurAnalogMode ( LX_ATV_AUDIO_MODE_GET_T *pAudioMode )
{
	...
	switch ( ui8SysSel )
	{
		...
		{
			/* BTSC US Only */
			if ( g_eAadInitType == LX_AAD_INIT_ATSC_BTSC_US_SELECT )	 //FM-Stereo + SAP (BTSC)
			{
				/* Do not use ui8SecCarDet flag because ui8SecCarDet flag does not set in BTSC BR signal.(2013.03.09) */
				if (ui8PrmCarDet == 1)
				{
					if ((ui8BiSap == 1) && (ui8MonoSt == 1) && (ui8SecCarDet == 1))	//SAP and Stereo
					{
					...
					//else if (ui8MonoSt == 1 && ui8SecCarDet == 1)	//Stereo
			...
			/* BTSC Other */
			else if ( g_eAadInitType == LX_AAD_INIT_ATSC_BTSC_SELECT )	 //FM-Stereo + SAP (BTSC)
			{
				/* Do not use ui8SecCarDet flag because ui8SecCarDet flag does not set in BTSC BR signal.(2013.03.09) */
				if (ui8PrmCarDet == 1)
				{
...
{
#ifndef ENABLE_M14_BTSC_50KHZ_FILTER
	...
	return RET_OK;
#else	//#ifndef ENABLE_M14_BTSC_50KHZ_FILTER
	//Check a BTSC mode.
	if( g_eAadInitType == LX_AAD_INIT_ATSC_BTSC_SELECT || g_eAadInitType == LX_AAD_INIT_ATSC_BTSC_US_SELECT )
	{
		// AAD Monitor Task Start for KOREA A2 Signal
		...
		AAD_DEBUG("AAD_SIF_SetTaskSatus : g_suiAadWaitQForMonitor = %d\n", g_suiAadWaitQForMonitor);
		return RET_OK;
	}
	else
	{
...
#endif	//#ifndef ENABLE_M14_BTSC_50KHZ_FILTER
}
...
int AAD_SIF_SetAADSIFInputPort(LX_AAD_SIF_INPUT_T sifSource)
{
	...
	AAD_ERROR("AAD_SIF_SetAADSIFInputPort : sifSource = %d\n", sifSource);
	return RET_OK;
}
...
/**
 * Check a PAL D/K Signal Strength After ASD fails to Scan Audio Signal.
 * @see
*/
static int AAD_DetectSoundSystem( LX_AAD_SIF_SOUNDSYSTEM_T eSoundSystem )
{
	...
	if ( g_eAadInitType == LX_AAD_INIT_DVB_ID_SELECT )
	{
		...
		if ( ui8PrmCarDet == 1 )
		{
			AAD_ERROR("PAL Manual Scan : Primary Carrier Detected\n");
		}
		else if ( ui8Sndstat == 0x2 )	//NICAM(digital sound) is available.
		{
			AAD_ERROR("PAL Manual Scan : NICAM Detected\n");
		}
		else
		{
			AAD_ERROR("PAL Manual Scan : No Primary Carrier & NICAM\n");
	...
	{
		//Check a Primary Carrier Status to detect PAL D/K Signal
		if ( ui8PrmCarDet == 1 )
		{
			AAD_ERROR("PAL Manual Scan : Primary Carrier Detected\n");
			...
			||(g_eAadInitType == LX_AAD_INIT_DVB_AJJA_SELECT) )
	{
		...
		if(g_eHighDevMode.bHighDevOnOff == TRUE &&  bNicamMode == FALSE)
		{
			...
			else if(g_eHighDevMode.highDevLevel == LX_AAD_HIGH_DEV_LEVEL_3)
			{
				...
				///////////////////////////////////////////////////////////
				//temp version to fix CN model issue.
	...
	else if ( g_eAadInitType == LX_AAD_INIT_DVB_IN_SELECT )
	{
		...
		else
		{
			...
			if (lx_chip() == LX_CHIP_M14)
			{
				...
				AAD_Wr01(manual_out_lvl_left_ch,  levlft, 0x30); // HDEV option : Off
				AAD_Wr01(manual_out_lvl_right_ch, levrt,  0x30);
...
static int AAD_SECAM_L_GetManualVolumeForAdjust ( void )
{
	...
	if (AAD_Rd(manual_out_lvl_left_ch) != ui8OutputVolume)
	{
...
/**
 * Get a SECAM L Manual Volume For Adjust after check SIF Magnitude for H13.
 * @see
*/
static int AAD_SECAM_L_SetManualVolumeForAdjust_H13 ( void )
{
	...
	/* Check a SIF Magnitude. */
	if( ui16Carrier1Magnitude < g_ui32AadSECAMVolume_H13[0] )
		ui8OutputVolume = 0xB7;
	else if( ui16Carrier1Magnitude >= g_ui32AadSECAMVolume_H13[0] && ui16Carrier1Magnitude < g_ui32AadSECAMVolume_H13[1] )
		ui8OutputVolume = 0xB8;
	else if( ui16Carrier1Magnitude >= g_ui32AadSECAMVolume_H13[1] && ui16Carrier1Magnitude < g_ui32AadSECAMVolume_H13[2] )
		ui8OutputVolume = 0xB5;
	else if( ui16Carrier1Magnitude >= g_ui32AadSECAMVolume_H13[2] && ui16Carrier1Magnitude < g_ui32AadSECAMVolume_H13[3] )
		ui8OutputVolume = 0xB2;
	else if( ui16Carrier1Magnitude >= g_ui32AadSECAMVolume_H13[3] && ui16Carrier1Magnitude < g_ui32AadSECAMVolume_H13[4] )
		ui8OutputVolume = 0xAF;
	else if( ui16Carrier1Magnitude >= g_ui32AadSECAMVolume_H13[4] && ui16Carrier1Magnitude < g_ui32AadSECAMVolume_H13[5] )
		ui8OutputVolume = 0xAC;
	else if( ui16Carrier1Magnitude >= g_ui32AadSECAMVolume_H13[5] && ui16Carrier1Magnitude < g_ui32AadSECAMVolume_H13[6] )
		ui8OutputVolume = 0xAA;
	else if( ui16Carrier1Magnitude >= g_ui32AadSECAMVolume_H13[6] && ui16Carrier1Magnitude < g_ui32AadSECAMVolume_H13[7] )
		ui8OutputVolume = 0xA8;
	else if( ui16Carrier1Magnitude >= g_ui32AadSECAMVolume_H13[7] && ui16Carrier1Magnitude < g_ui32AadSECAMVolume_H13[8] )
		ui8OutputVolume = 0xA6;
	else if( ui16Carrier1Magnitude >= g_ui32AadSECAMVolume_H13[8] && ui16Carrier1Magnitude < g_ui32AadSECAMVolume_H13[9] )
		ui8OutputVolume = 0xA4;
	else if( ui16Carrier1Magnitude >= g_ui32AadSECAMVolume_H13[9] && ui16Carrier1Magnitude < g_ui32AadSECAMVolume_H13[10] )
		ui8OutputVolume = 0xA2;
	else if( ui16Carrier1Magnitude >= g_ui32AadSECAMVolume_H13[10] && ui16Carrier1Magnitude < g_ui32AadSECAMVolume_H13[11] )
		ui8OutputVolume = 0xA1;
	else if( ui16Carrier1Magnitude >= g_ui32AadSECAMVolume_H13[11] && ui16Carrier1Magnitude < g_ui32AadSECAMVolume_H13[12] )
		ui8OutputVolume = 0xA0;
	else if( ui16Carrier1Magnitude >= g_ui32AadSECAMVolume_H13[12] && ui16Carrier1Magnitude < g_ui32AadSECAMVolume_H13[13] )
		ui8OutputVolume = 0x9F;
	else if( ui16Carrier1Magnitude >= g_ui32AadSECAMVolume_H13[13] && ui16Carrier1Magnitude < g_ui32AadSECAMVolume_H13[14] )
		ui8OutputVolume = 0x9E;
	else
		ui8OutputVolume = 0x9D;

	/* Check a previous volume. */
	if (AAD_Rd(manual_out_lvl_left_ch) != ui8OutputVolume && g_ui32AadSECAMDisable_H13 == FALSE)
	{
		AAD_Wr01(manual_out_lvl_left_ch,  levlft, ui8OutputVolume);
		AAD_Wr01(manual_out_lvl_right_ch, levrt,  ui8OutputVolume);
		...
		AAD_DEBUG("SetManualVolumeForAdjust_H13 : Magnitude 0x%4X, Volume = 0x%X\n",	\
...
int AAD_Monitor_Task( void *i_pvParam )
{
	...
	do
	{
		...
		else if ( (g_eAadInitType == LX_AAD_INIT_DVB_IN_SELECT && lx_chip() == LX_CHIP_M14) \
				&&(g_bAadSuspended == FALSE) )
		{
				 ...
				 &&(g_bAadSuspended == FALSE) )
		{
			...
			else if(g_i_eSystem == LX_AAD_SIF_SYSTEM_L)
			{
				...
				else 	//if (lx_chip_rev() >= LX_CHIP_REV(H13, A0))
				{
					...
					{
						/* Check a SIF SECAM L Magnitude. */
						(void)AAD_SECAM_L_SetManualVolumeForAdjust_H13();
					}
...
static int AAD_PAL_B_A2_India_Monitor_Task( void )
{
	  ...
	  ||(g_ui8PALBContiCountOn == HIGH_DEV_OFF_TO_ON_COUNT_IN) )
	{
		...
		if(eHighDevMode.bHighDevOnOff != g_eHighDevMode.bHighDevOnOff)
		{
			...
			if(ui8HighDevStatus == HIGH_DEV_ON_STATUS)
			{
				...
				AAD_ERROR( "AAD> Set HDev1 540Khz(%2d->%2d(%2d)) = %d->%d, QUAL1 = 0x%4X, CAR2 = 0x%4X, DIFF = 0x%4X\n",	\
			...
			else
			{
				...
				AAD_ERROR( "AAD> Set HDev 50Khz(%2d->%2d(%2d)) = %d->%d, QUAL1 = 0x%4X, CAR2 = 0x%4X, DIFF = 0x%4X\n",	\
							g_ui8PALBTaskCount, g_ui8PALBContiCount, g_ui8PALBContiCountOn, g_ui8HighDevEabled, ui8HighDevStatus,	\
							ui16Carrier1Quality, ui16Carrier2Magnitude, ui16Quality1ToMag2);
			}
		}
		   ...
		   ||(g_ui8PALBTaskCount == (HIGH_DEV_TASK_MAX_IN-1)  && g_ui8HighDevEabled != HIGH_DEV_OFF_STATUS) )
	{
		...
		AAD_ERROR( "AAD> Set HDev2 540Khz(%2d->%2d(%2d)) = %d->%d, QUAL1 = 0x%4X, CAR2 = 0x%4X, DIFF = 0x%4X\n", \
					g_ui8PALBTaskCount, g_ui8PALBContiCount, g_ui8PALBContiCountOn, g_ui8HighDevEabled, ui8HighDevStatus,	\
					ui16Carrier1Quality, ui16Carrier2Magnitude, ui16Quality1ToMag2);
	}
...
static int AAD_SetBTSCDemodFilter( void )
{
	...
	if ( g_i_eSifSource == LX_AAD_SIF_INPUT_INTERNAL )
	{
		...
		else if (lx_chip_rev() >= LX_CHIP_REV(M14, A0))
		{
			...
			// AAD Demod Filter 100Khz for BTSC Signal
			if( g_suiAadWaitQForMonitor == LX_AAD_TASK_START )
			{
				/* Set High Deviation On or Off Status and Level */
				AAD_Wr01(fm_am_demod_ctrl, dmd1dv,  0x1);
				AAD_Wr01(fm_am_demod_ctrl, dmd1fl,  0x1);
			}
			else	//AAD Demod Filter 50Khz for BTSC Signal
			{
				AAD_Wr01(fm_am_demod_ctrl, dmd1dv,  0x0);
				AAD_Wr01(fm_am_demod_ctrl, dmd1fl,  0x0);
			}
		...
		AAD_DEBUG("AAD_SetBTSCDemodFilter(INTERNAL): fm_am_demod_ctrl = 0x%X\n", AAD_Rd(fm_am_demod_ctrl));
	}
	else
	{
		//Get high deviation level using FM AM demod 1 control register
		...
		AAD_DEBUG("AAD_SetBTSCDemodFilter(EXTERNAL): fm_am_demod_ctrl = 0x%X\n", AAD_Rd(fm_am_demod_ctrl));
	}
...
//For later use only
	...
	//Read SNR value for Carrier 1 Quality and Magnitude
	AAD_RdFL(car1_fm_qual0);
	AAD_Rd01(car1_fm_qual0, cr1ql, value);
	ui16Carrier1Quality  = value << 8;
	...
	AAD_Rd01(car1_fm_qual1, cr1ql, value);
	ui16Carrier1Quality += value;
	AAD_ERROR("Carrier1 Quality Current   0x%4X(Threshold	0x%4X)\n", ui16Carrier1Quality, (CARRIER1_QUAL_AAOS_THD_HI_DVB << 3) );
	...
	//Check a Primary Carrier Status and Quality Threshold to detect PAL D/K Signal
	if ( (ui8PrmCarDet == 1) && (ui16Carrier1Quality >= (CARRIER1_QUAL_AAOS_THD_HI_DVB << 3)) )
	{
		AAD_ERROR("PAL D/K : Primary Carrier & SNR Detected\n");

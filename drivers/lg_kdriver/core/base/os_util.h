...
typedef enum
{
	OS_RGN_MGR_TYPE_TLSF	= 0x0,	///< memory manager by TLSF 
...
#define OS_LockMutexEx(pSem,timeout)	OS_LockMutex_Tag(pSem,timeout, __FL__, __F__, __L__)
#define OS_LockMutex(pSem)				OS_LockMutex_Tag(pSem,OS_INFINITE_WAIT, __FL__, __F__, __L__)
#define OS_UnlockMutex(pSem)			OS_UnlockMutex_Tag(pSem, __FL__, __F__, __L__)
...
#define OS_RecvEvent(pEvnt,ev,rEv,opt,tm)	OS_RecvEvent_Tag(pEvnt,ev,rEv,opt,tm,__FL__,__F__,__L__)
#define	OS_SendEvent(pEvnt,ev)		OS_SendEvent_Tag( pEvnt, ev, __FL__, __F__, __L__)
#define OS_ClearEvent(pEvnt)		OS_ClearEvent_Tag( pEvnt, __FL__, __F__, __L__)
 ...
 *	@return RET_OK if success, RET_ERROR otherwise.
 */
#define OS_KMalloc(size)			OS_KMalloc_Tag(size, __FL__, __F__, __L__)
#define OS_KFree(ptr)				OS_KFree_Tag(ptr, __FL__, __F__, __L__)
...
#define OS_InitRegionEx(pRgn,mgr_type,phys_addr,size,block_size) \
									OS_InitRegionEx_Tag(pRgn, mgr_type, (void*)phys_addr, size, block_size, __FL__, __F__, __L__ )
...
#define OS_CleanupRegion(pRgn)		OS_CleanupRegion_Tag(pRgn, __FL__, __F__, __L__)
#define OS_MallocRegion(pRgn,size)	OS_MallocRegion_Tag(pRgn,size, __FL__, __F__, __L__)
#define OS_FreeRegion(pRgn,ptr)		OS_FreeRegion_Tag(pRgn,ptr, __FL__, __F__, __L__)
#define OS_GetRegionInfo(pRgn,pMemInfo)	OS_GetRegionInfo_Tag(pRgn, pMemInfo, __FL__, __F__, __L__)
 ...
 */
#define OS_OpenFile(pFile,fname,flags,mode)		OS_OpenFile_Tag(pFile, fname, flags, mode, __FL__, __F__, __L__)
#define OS_CloseFile(pFile)						OS_CloseFile_Tag(pFile , __FL__, __F__, __L__)
#define OS_SeekFile(pFile, offset, whence)		OS_SeekFile_Tag(pFile, offset, whence, __FL__, __F__, __L__)
#define OS_SizeOfFile(pFile)					OS_SizeOfFile_Tag(pFile,  __FL__, __F__, __L__)
#define OS_WriteFile(pFile,buffer,count)		OS_WriteFile_Tag(pFile, buffer, count,  __FL__, __F__, __L__)
#define OS_ReadFile(pFile,buffer,count)			OS_ReadFile_Tag(pFile, buffer, count,  __FL__, __F__, __L__)
 ...
 *
 */
#define	OS_FAILURE()				OS_FAILURE_Tag(__FL__, __F__, __L__)

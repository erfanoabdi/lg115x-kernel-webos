...
static int			g_fbdev_use_isr_mon	= 0;
module_param_named( fbdev_isr_mon, g_fbdev_use_isr_mon, int, 0644 );

/* if enabled, UHD OSD display test enabled */
int  				g_fbdev_uhd_disp_test = 0x0;
module_param_named( fbdev_uhd_disp_test, g_fbdev_uhd_disp_test, int, 0644 );
...
static int FBDEV_pan_display ( struct fb_var_screeninfo* var, struct fb_info* fbinfo )
{
	 ...
	 */
#if 0
	offset	+= ( var->yoffset + wctx->pixel.input_win.y ) * fbinfo->fix.line_length;
	offset	+= ( var->xoffset + wctx->pixel.input_win.x ) * var->bits_per_pixel / 8;
...
#ifndef	_DEBUG_
	if (!request_mem_region(wctx->osd_hdr_base_phys, wctx->osd_hdr_size, fbname))
	{
		FBDEV_DEBUG("%s: can't reserve osd header region\n", fbname);
		goto err_exit;
	}
#endif
	...
	if ( g_fbdev_use_isr_mon )
	{
		...
		else
		{
			if ( ++g_fbdev_vsync_cnt & 0xff00 ) /* vsync count >= 256 */
			{
				...
				printk("[fb] ISR - vsync_tm %d us (%d jiffies), diff_tm %d ms/256, HZ=%d\n",
						g_fbdev_vsync_timeout, (UINT32)usecs_to_jiffies(g_fbdev_vsync_timeout), diff_tick, HZ );
...
static void	FBDEV_RestoreFBImage( UINT32* virt_img, UINT32 fbmem_addr, UINT32 length )
{
	OS_CACHE_MAP_T cmap;
...
void FBDEV_RestoreFBList(void)
{
	...
	{
		FBDEV_RestoreFBImage( fbmem_bkup_list[i].virt_img, fbmem_bkup_list[i].phys_addr, fbmem_bkup_list[i].length);
...
static int FBDEV_suspend(struct platform_device *pdev, pm_message_t state)
{
	FBDEV_DRVDATA_T* drv_data = platform_get_drvdata(pdev);
...
static int FBDEV_resume(struct platform_device *pdev)
{
	...
	FBDEV_INFO("fb resume device...\n");

	FBDEV_CHECK_ERROR( drv_data->b_suspend == 0, return RET_OK, "invalid resume call\n");

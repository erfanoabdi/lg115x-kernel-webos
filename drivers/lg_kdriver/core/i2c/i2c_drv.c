...
static int I2C_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	{
		printk("[%s]{Hibernation] I2C_Open is not done.. skip suspend\n",I2C_MODULE);
		return 0;
	}
	...
	I2C_PRINT_ERROR("[%s] done suspend\n", I2C_MODULE);
...
static int I2C_resume(struct platform_device *pdev)
{
	...
	if(g_iic_open_done == false)
	{
		printk("[%s]{Hibernation] I2C_Open is not done.. skip resume\n",I2C_MODULE);
		return 0;
	}

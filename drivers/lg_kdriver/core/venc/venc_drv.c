...
int VENC_Init(void)
{
	...
	venc_irq_num = gpstVencConfig->irq_num;

    if ( request_irq(venc_irq_num, (irq_handler_t)VENC_Interrupt, 0, "DTVSoC2VENC", g_venc_device)) {
...
void VENC_Cleanup(void)
{
	...
	free_irq( venc_irq_num, g_venc_device);
...
VENC_Ioctl ( struct file *filp, unsigned int cmd, unsigned long arg )
{
	...
	switch(cmd)
	{
		...
		case LX_VENC_IOW_RECORD_SET_RC:
		{
			...
			ret = VENC_HAL_SetRateControl( &stParam );
		...
		case LX_VENC_IOWR_ENC_CREATE:
		{
			...
			ret = VENC_HAL_ENC_Create( &stParam );
		...
		case LX_VENC_IOW_ENC_DESTROY:
		{
			...
			ret = VENC_HAL_ENC_Destroy( &stParam );
		}
		break;
		...
		case LX_VENC_IOWR_ENC_ENCODE:
		{
			...
			ret = VENC_HAL_ENC_Encode( &stParam );

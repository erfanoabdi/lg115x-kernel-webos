...
void SDEC_PES_SetVirtOffset(void)
{
	...
	baseAddr 	= gMemCfgSDECGPB[pSdecConf->memCfg].gpb_memory_base;
	size		= gMemCfgSDECGPB[pSdecConf->memCfg].gpb_memory_size;
...
int SDEC_PES_ParsePesHeader(UINT32 va_gpbRdPtr, UINT8 payloadOffset, PesHeader *pes_header, UINT32 va_gpbBaseP, UINT32 va_gpbEndPt)
{
	...
	{
		SDEC_DTV_SOC_Message( SDEC_PES, "No Prefix!!!!!!");
		return 1;
	}
...
void SDEC_PES_ParseTsHeader(UINT32 va_gpbRdPtr, TsHeader *ts_header, UINT32 va_gpbBaseP, UINT32 va_gpbEndPt)
{
	...
	if(_gPrintEnable)
	{
		SDEC_DTV_SOC_Message( SDEC_PES, "PID : 0x%x HEADER[%02x%02x%02x%02x%02x]", ts_header->PID, buff[0], buff[1], buff[2], buff[3], buff[4]);
		SDEC_DTV_SOC_Message( SDEC_PES, "[%d]: transport_error_indicator", ts_header->transport_error_indicator);
		SDEC_DTV_SOC_Message( SDEC_PES, "[%d]: payload_unit_start_indicator", ts_header->payload_unit_start_indicator);
		SDEC_DTV_SOC_Message( SDEC_PES, "[%d]: transport_priority", ts_header->transport_priority);
		SDEC_DTV_SOC_Message( SDEC_PES, "[%d]: transport_scrambling_control", ts_header->transport_scrambling_control);
		SDEC_DTV_SOC_Message( SDEC_PES, "[%d]: adaptation_field_control", ts_header->adaptation_field_control);
		SDEC_DTV_SOC_Message( SDEC_PES, "[%d]: continuity_counter", ts_header->continuity_counter);
	}
	...
	if (ts_header->transport_error_indicator == 0 && (ts_header->adaptation_field_control & 0x02) ) // '10' or '11'
	{
		...
		{
			;
			SDEC_DTV_SOC_Message( SDEC_PES, "adaptaion field length : %d", ts_header->adaptation_field_length);
		}
	}
...
void SDEC_PES_CopyPacket(S_SDEC_PARAM_T *stpSdecParam, UINT8 chIdx, UINT8 gpbIdx, UINT32 va_gpbRdPtr, UINT8 payloadOffset)
{
	...
	//SDEC_DTV_SOC_Message( SDEC_PES, "1 gp_rp[%08x] dt_wp[%08x]", va_gpbRdPtr,  va_dstWrPtr);
	...
	if( (va_gpbRdPtr + sizeToCopy) > va_gpbEndPt)
	{
		...
		//SDEC_DTV_SOC_Message( SDEC_PES, "SIZE[%4d] Wraped[%4d]", remainSize, wrappedSize);
		//SDEC_DTV_SOC_Message( SDEC_PES, "1 gp_rp[%08x] dt_wp[%08x]", va_gpbRdPtr,  va_dstWrPtr);
		...
		if( ( va_dstWrPtr + wrappedSize ) <= va_dstEndPt)
		{
			...
			//SDEC_DTV_SOC_Message( SDEC_PES, "Normal copy");
		}
		else
		{
			/*
			...
			//SDEC_DTV_SOC_Message( SDEC_PES, "Segment Copy tailSize[%x]", tailSize);
		}

		//SDEC_DTV_SOC_Message( SDEC_PES, "2 gp_rp[%08x] dt_wp[%08x]", va_gpbRdPtr, va_dstWrPtr);
		...
		//SDEC_DTV_SOC_Message( SDEC_PES, "3 gp_rp[%08x] dt_wp[%08x]", va_gpbRdPtr, va_dstWrPtr);
	...
	else
	{
		...
		//SDEC_DTV_SOC_Message( SDEC_PES, "SIZE[%4d]", remainSize);
		//SDEC_DTV_SOC_Message( SDEC_PES, "1 gp_rp[%08x] dt_wp[%08x]", va_gpbRdPtr, va_dstWrPtr);
...
void SDEC_PES_Proc(struct work_struct *work)
{
	...
	SDEC_DTV_SOC_Message( SDEC_TRACE, "<--SDEC_PES_Proc");

	ui8PesGpbInfoWIdx = stpSdecParam->ui8PesGpbInfoWIdx;
	ui8PesGpbInfoRIdx = stpSdecParam->ui8PesGpbInfoRIdx;

	//SDEC_DTV_SOC_Message( SDEC_PES, "SDEC_PES_Proc");
	//SDEC_DTV_SOC_Message( SDEC_PES, "ui8PesGpbInfoRIdx[%d], ui8PesGpbInfoWIdx[%d]", stpSdecParam->ui8PesGpbInfoRIdx, stpSdecParam->ui8PesGpbInfoWIdx);
	...
	while(ui8PesGpbInfoRIdx != ui8PesGpbInfoWIdx)
	{
		...
		if(_SDEC_PES_IsPESFltIdx(chIdx, gpbIdx) == FALSE )
		{
			...
			SDEC_DEBUG_Print("in PES_Proc, Filter Indx is not valid: ch[%d] idx[%d]", chIdx, gpbIdx);
			//continue;
			goto tail_while;
		...
		//SDEC_DTV_SOC_Message( SDEC_PES, "CH%c GPB Idx[%d] rp[%08x] wp[%08x]", (chIdx)?'B':'A', gpbIdx , gpbRdPtr, gpbWrPtr);

		if( !gpbBase || !gpbEnd || !gpbRdPtr || !gpbWrPtr)
		{
			SDEC_DEBUG_Print("in PES_Proc, address is not valid : B[%08x] E[%08x] R[%08x] W[%08x]", gpbBase, gpbEnd, gpbRdPtr, gpbWrPtr);
		...
		//SDEC_DTV_SOC_Message( SDEC_PES, "remainSize[%d] r[%08x] w[%08x]", remainSize, gpbRdPtr, gpbWrPtr);
		...
		while(remainSize > 0)
		{
			...
			if( tsHeader.sync_byte == 0x47 && payloadOffset < 188 )
			{
				...
				if( tsHeader.payload_unit_start_indicator == 1)
				{
					...
					{
						//SDEC_DTV_SOC_Message( SDEC_PES, "PES Packet Length is %d",pesHeader.PES_packet_length);
				...
				{
					//SDEC_DTV_SOC_Message( SDEC_PES, "cc[%d]", tsHeader.continuity_counter);
					...
					if( tsHeader.continuity_counter == _gPesInfo[chIdx][gpbIdx].continuity_count )
					{
						...
						//SDEC_DTV_SOC_Message( SDEC_PES, "copied %d", TS_PACKET_SIZE - payloadOffset);
						...
						{
							SDEC_DTV_SOC_Message( SDEC_PES, "copied[%5d] pesPacket[%5d]", _gPesInfo[chIdx][gpbIdx].copiedSize,  _gPesInfo[chIdx][gpbIdx].pesPacketSize);
					...
					{
						SDEC_DTV_SOC_Message( SDEC_ERROR, "Continuity counter error cc[%2d] orgin[%2d]", tsHeader.continuity_counter, _gPesInfo[chIdx][gpbIdx].continuity_count);
						_gPesInfo[chIdx][gpbIdx].copiedSize 		= 0;
						_gPesInfo[chIdx][gpbIdx].isStart 			= 0;
						_gPesInfo[chIdx][gpbIdx].continuity_count 	= 0;
						_gPesInfo[chIdx][gpbIdx].gpbRdPtr 			= _gPesInfo[chIdx][gpbIdx].gpbWrPtr;
...
static BOOLEAN _SDEC_PES_SetProcParam(S_SDEC_PARAM_T *stpSdecParam, S_GPB_INFO_T procParam)
{
	...
	SDEC_DTV_SOC_Message( SDEC_TRACE, "<--_SDEC_PES_SetProcParam");
	...
	else
	{
		...
		stpSdecParam->ui8PesGpbInfoRIdx = 0;
		stpSdecParam->ui8PesGpbInfoWIdx = 0;
		SDEC_DEBUG_Print( "SDEC_PES : Full!");
	}

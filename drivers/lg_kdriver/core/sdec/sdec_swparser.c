...
void SDEC_SWP_Init(S_SDEC_PARAM_T *stpSdecParam)
{
	...
	if(stSWTSParser_Thread)
	{
		...
		SDEC_DTV_SOC_Message( SDEC_NOTI,"Run SDEC SWTS Parser Thread PID:[%d]", stSWTSParser_Thread->pid);
	}
	else
	{
		SDEC_DTV_SOC_Message( SDEC_NOTI,"SDEC SWTS Parser Thread Already Created");
	}
...
void SDEC_SWP_ComponentInit(S_SDEC_PARAM_T *stpSdecParam)
{
	...
	if(_gVirtAddr != 0)
	{
		...
		SDEC_DTV_SOC_Message( SDEC_SWPARSER, "iounmapped previous ioremap pointer");
	}
#endif
	...
	if(ret != RET_OK)
	{
		SDEC_DEBUG_Print("SDEC_MM_Alloc Error in SWTSParser Thread [%d]",ret);
		return;
	}
...
//	if (lx_chip_rev() >= LX_CHIP_REV(H13, A0)) 
	...
	if( ( lx_chip_rev() >= LX_CHIP_REV(M14, B0) ) && ( lx_chip_rev() < LX_CHIP_REV(H14, A0) ) )
	{
		...
		baseAddr = gMemCfgDvr[1][LX_PVR_MEM_DN].base;
		size	 = gMemCfgDvr[1][LX_PVR_MEM_DN].size;
	...
	else
	{
		if(ch == 0)
		{
		baseAddr = gMemCfgDvr[2][LX_PVR_MEM_DN].base;
		size	 = gMemCfgDvr[2][LX_PVR_MEM_DN].size;
		}
		else
		{
			baseAddr = gMemCfgDvr[2][LX_PVR_MEM_DN1].base;
			size	 = gMemCfgDvr[2][LX_PVR_MEM_DN1].size;
		}
	...
	SDEC_DTV_SOC_Message( SDEC_SWPARSER, "SDT DN_BUF [0x%x] size [0x%x]", baseAddr, size);		
	SDEC_DTV_SOC_Message( SDEC_SWPARSER, "virt addr [0x%x]", _gVirtAddr);
	SDEC_DTV_SOC_Message( SDEC_SWPARSER, "virt offset [0x%x]", _gVirtOffset);
...
void SDEC_SWP_SetSectionFilterPattern(LX_SDEC_SECFLT_PATTERN_T *stpLXSecFltPattern)
{
	...
	SDEC_DTV_SOC_Message( SDEC_TRACE, "<--SDEC_SWP_SetSectionFilterPattern");
...
void SDEC_SWP_SetSectionBuffer(S_SDEC_PARAM_T *stpSdecParam)
{
	...
	SDEC_DTV_SOC_Message( SDEC_TRACE, "<--SDEC_SWP_SetSectionBuffer");
	...
	SDEC_DTV_SOC_Message( SDEC_SWPARSER, "baseAddr[0x%x]", baseAddr);
	SDEC_DTV_SOC_Message( SDEC_SWPARSER, "size[0x%x]", size);
	...
	SDEC_DTV_SOC_Message( SDEC_SWPARSER, "virtAddr[0x%x]", _gSecCirVirtAddr);
	SDEC_DTV_SOC_Message( SDEC_SWPARSER, "_gSecCirVirtOffset[0x%x]", _gSecCirVirtOffset);
	...
	SDEC_DTV_SOC_Message( SDEC_SWPARSER, "gSWTSFilter[0].pui8BufStart[0x%x]", (UINT32)gSWTSFilter[0].pui8BufStart);
	SDEC_DTV_SOC_Message( SDEC_SWPARSER, "gSWTSFilter[0].pui8ReadPtr[0x%x]", (UINT32)gSWTSFilter[0].pui8ReadPtr);
	SDEC_DTV_SOC_Message( SDEC_SWPARSER, "gSWTSFilter[0].pui8WritePtr[0x%x]", (UINT32)gSWTSFilter[0].pui8WritePtr);
	SDEC_DTV_SOC_Message( SDEC_SWPARSER, "gSWTSFilter[0].pui8BufEnd[0x%x]", (UINT32)gSWTSFilter[0].pui8BufEnd);
...
void SDEC_SWP_SetPID(UINT32 ui32PidFltId, UINT32 ui32PidValue)
{
	...
	if(ui32PidFltId != 0)
	{
		SDEC_DEBUG_Print("ERROR: SDEC SWTS Parser Set PID with PIDFLT Index [%d] \n",ui32PidFltId);
	}
 ...
 * @see
 */
DTV_STATUS_T SDEC_SWP_EnableSWParser
	...
	LX_SDEC_SECFLT_ENABLE_SWPARSER_T *stpLXSdecSecfltEnableSWParser)
{
	...
	LX_SDEC_CHECK_CODE( stpLXSdecSecfltEnableSWParser == NULL, return INVALID_ARGS, "Invalid argument" );

	OS_LockMutex(&stpSdecParam->stSdecMutex);

	SDEC_DTV_SOC_Message( SDEC_TRACE, "<--SDEC_SWP_EnableSWParser");
	...
	if(bEnable)
	{
		SDEC_DTV_SOC_Message( SDEC_SWPARSER, "Send START to SDEC SWTS Parser Thread CH[%d] \n", ui8Ch);
		/* Set H13 SDT Channel */
	...
	{
		SDEC_DTV_SOC_Message( SDEC_SWPARSER, "Send STOP to SDEC SWTS Parser Thread CH[%d] \n", ui8Ch);
...
static int SDEC_SWP_ProcTsPacket(UINT8 *pBuffer, TsPacket *tsPacket)
{
	...
	{
		SDEC_DEBUG_Print("ERROR NO_PAYLOAD");
		return -NO_PAYLOAD;
	}
...
static int SDEC_SWP_CheckTSError(TsPacket *tsPacket)
{
	...
	{
		SDEC_DEBUG_Print("Payload Size is 0");
	...
	{
		SDEC_DEBUG_Print("PID is not matched [0x%x][0x%x]\n",gSWTSFilter[0].ui16PID,tsPacket->ui16PID);
		return -PID_ERROR;
	}
	...
	else
	{
		...
		{
			SDEC_DEBUG_Print("Same CC, Duplicated TS");
		...
		if(gSWTSFilter[0].ui8Continuity_counter != tsPacket->ui8Continuity_counter)
		{
			SDEC_DEBUG_Print("CC Mismatched [%d][%d]",gSWTSFilter[0].ui8Continuity_counter, tsPacket->ui8Continuity_counter);
			gSWTSFilter[0].ui8Continuity_counter = tsPacket->ui8Continuity_counter; /* Set with New CC */
			return -CC_ERROR;
		}
	}
...
static int SDEC_SWP_SectionFiltering(UINT8 *pBuffer)
{
	...
	{	
		...
		{
			SDEC_DTV_SOC_Message( SDEC_SWPARSER, "NOT Ver. NG DATA[0x%x] PATTERN[0x%x]", ui8Version, ui8Pattern);
			return -1;
		}
	}
	...
	if(gSWTSFilter[0].ui32Mask_H != 0)
	{
		...
		{
			SDEC_DTV_SOC_Message( SDEC_SWPARSER, "H NG DATA[0x%08x] PATTERN[0x%08x] MASK[0x%08x]", ui32Hdata, gSWTSFilter[0].ui32Pattern_H, gSWTSFilter[0].ui32Mask_H );
			return -1;
		}
	}
	...
	if(gSWTSFilter[0].ui32Mask_L != 0)
	{
		...
		{
			SDEC_DTV_SOC_Message( SDEC_SWPARSER, "L NG DATA[0x%08x] PATTERN[0x%08x] MASK[0x%08x]", ui32Ldata, gSWTSFilter[0].ui32Pattern_L, gSWTSFilter[0].ui32Mask_L );
			return -1;
		}
	}
...
static UINT32 SDEC_SWP_GetCRC32 (UINT8 *pBuffer)
{
	...
	SDEC_DTV_SOC_Message( SDEC_SWPARSER, "CRC Check Section Data 0-8");
	SDEC_DTV_SOC_Message( SDEC_SWPARSER, "[%02x][%02x][%02x][%02x][%02x][%02x][%02x][%02x]",
								pBuffer[0],pBuffer[1],pBuffer[2],pBuffer[3]
								,pBuffer[4],pBuffer[5],pBuffer[6],pBuffer[7]);
...
static int SDEC_SWP_CheckCRC(UINT8 *pDataPtr)
{
	...
	SDEC_DTV_SOC_Message( SDEC_SWPARSER, "CRC[0x%x] CALCRC[0x%x]", crc32, calcCrc32);
	
	if( calcCrc32 != crc32 )
	{
		SDEC_DEBUG_Print("CRC Error [%x][%x]",crc32,calcCrc32);
		return -SECTION_CRC;
	}
...
BOOLEAN SDEC_SWP_Section_Proc(S_SDEC_PARAM_T *stpSdecParam, UINT8 ui8Ch, UINT8 ui8SecFltIdx)
{
	...
	SDEC_DTV_SOC_Message( SDEC_ISR, "GPB_DATA_CHC");
	...
	{
		SDEC_DTV_SOC_Message( SDEC_ERROR, "CHC GPB_Data GPB BND ERROR Idx[%d] b[%08x] w[%08x] e[%08x]",
	...
	SDEC_DTV_SOC_Message( SDEC_ISR, "CHC GPB_Data GPB Idx[%d] rp[%d] wp[%d]",
	...
	if( ui32GpbRdPtr < ui32GpbWrPtr )
	{
		...
		{
			SDEC_DTV_SOC_Message( SDEC_ERROR, "1 CHC GPB_Data GPB Idx[%d] rp[%08x] wp[%08x] OVERFLOW",
	...
	else
	{
		...
		if( ui32GpbRdPtr < ui32UsrRdPtr ||
			ui32UsrRdPtr < ui32GpbWrPtr )
		{
			SDEC_DTV_SOC_Message( SDEC_ERROR, "2 CHC GPB_Data GPB Idx[%d] rp[%08x] wp[%08x] OVERFLOW",
								ui8SecFltIdx,ui32GpbRdPtr, ui32GpbWrPtr);
			/* check previous queue */
			SDEC_CheckNotifyOvf(stpSdecParam, &notiParam);
		}
	...
	{
		SDEC_DTV_SOC_Message( SDEC_ISR, "CHC GPB_Data GPB Idx[%d] one shot  mode",  ui8SecFltIdx);

		/* Get PID Filter index */
		ui8PidFltIdx = 0;
		
		SDEC_DTV_SOC_Message( SDEC_ISR, "CHC GPB_Data GPB Idx[%d] Disable pid filter %d", ui8SecFltIdx, ui8PidFltIdx);

		/* 2012. 11. 17 PVR_RELATED To Modify DN Channel B to A, PVR Use DN Channel B , SDT will use Ch A */
		/* 20140123 To change for supporting both channel in webOS UHD DVB */
		PVR_DD_StopDownload((LX_PVR_CH_T)(stpSdecParam->ui8SDTChannel));
...
static int _SDEC_SWTSParser_Thread(void *data)
{
	...
	while(!kthread_should_stop())
	{
		...
		if ( ret == RET_OK )
		{
			...
			{
				SDEC_DEBUG_Print("SDEC SWP OS_RecvEvent Error Wait[%d] Received[%d]",(UINT32)eWaitEvent, ui32ReceivedEvent );
				continue;
			}
			else
			{
				if( ui32ReceivedEvent == (UINT32)SWP_EVENT_START)	/* Received START */
				{
					...
					SDEC_DTV_SOC_Message( SDEC_SWPARSER, "SDEC Parser Thread Received Start CH[%d] \n",(UINT32)pvr_ch);
#if 0					
					...
					SDEC_DTV_SOC_Message( SDEC_SWPARSER, "Init DN Buf Start[0x%x]End[0x%x] VirS[0x%x]VirE[0x%x] VirR[0x%x]VirW[0x%x] \n", 
				...
				{
					SDEC_DTV_SOC_Message( SDEC_SWPARSER, "SDEC Parser Thread Received Stop \n");
		...
		uiSize = 0;
		if( RET_OK != PVR_DD_GetDownloadWriteAddr(pvr_ch, &ui32PhyDNWritePtr))
		{
			SDEC_DEBUG_Print("KADP_PVR_DnGetWriteAddr Error");
			continue;
		}
		...
		SDEC_DTV_SOC_Message(SDEC_SWPARSER, "PVR DN Write Addr = 0x%x ", (UINT32)ui32PhyDNWritePtr);
		SDEC_DTV_SOC_Message(SDEC_SWPARSER, "PVR DN Virtual Write Addr = 0x%x ", (UINT32)gSWTSFilter[0].pui8DNWritePtr);
		...
		if(gSWTSFilter[0].pui8DNWritePtr >= gSWTSFilter[0].pui8DNReadPtr)
		{
			...
			SDEC_DTV_SOC_Message(SDEC_SWPARSER, "Normal Wptr[0x%x] Rptr[0x%x] ", (UINT32)gSWTSFilter[0].pui8DNWritePtr, (UINT32)gSWTSFilter[0].pui8DNReadPtr);
		}
		else
		{
			/* Wrap Around Case */
			...
			SDEC_DTV_SOC_Message(SDEC_SWPARSER, "Wrap Wptr[0x%x] Rptr[0x%x] Sptr[0x%x] Eptr[0x%x] ", 
...
#endif
		SDEC_DTV_SOC_Message( SDEC_SWPARSER, "Get Data From PVR Buffer Size[%d]", uiSize);
		...
		while((uiSize - TS_PACKET_SIZE + 4) >= 0) /* + 4 is timestamp , temp */
		{
			...
			if(ret != 0)
			{
				SDEC_DEBUG_Print("SDEC_SWP_ProcTsPacket Error [%d]",ret);
				goto NEXT_TS;
			}

			SDEC_DTV_SOC_Message( SDEC_SWPARSER, "Get TS Payload Size[%d]", tsPacket.ui8PayloadSize);

			ret = SDEC_SWP_CheckTSError(&tsPacket);
			if(ret != 0)
			{
				SDEC_DEBUG_Print("SDEC_SWP_CheckTSError Error [%d]",ret);
				goto NEXT_TS;
			}
			...
			if(gSWTSFilter[0].eFilterState == SWP_FLTSTAT_SEC_HEADER_NEED)
			{
				if(tsPacket.ui8Payload_unit_start_indicator == 1)
				{
					...
					SDEC_DTV_SOC_Message( SDEC_SWPARSER, "PUSI Case, Cal again TS Payload with PUSI , Remain P Size[%d]", tsPacket.ui8PayloadSize);
				}
				else /* if(tsPacket.ui8Payload_unit_start_indicator != 1) */
				{
					SDEC_DTV_SOC_Message( SDEC_SWPARSER, "Waiting Payload Unit Start Indicator");
					goto NEXT_TS;
				}
			}
			else
...
parse_continuous_section:
			{
				...
				{
					SDEC_DTV_SOC_Message( SDEC_SWPARSER, "SWP_FLTSTAT_SEC_HEADER_ING");
					...
					if(ui8RemainSize >= gSWTSFilter[0].ui8NeedHeaderSize)
					{
						...
						SDEC_DTV_SOC_Message( SDEC_SWPARSER, "SWP_FLTSTAT_SEC_HEADER_DONE ui8RemainSize[%d]",ui8RemainSize);
						SDEC_DTV_SOC_Message( SDEC_SWPARSER, "Get Section Header");
						SDEC_DTV_SOC_Message( SDEC_SWPARSER, "[%02x][%02x][%02x][%02x][%02x][%02x][%02x][%02x]",gSWTSFilter[0].au8HeaderBuffer[0],gSWTSFilter[0].au8HeaderBuffer[1],
					...
					{
						SDEC_DTV_SOC_Message( SDEC_SWPARSER, "Remain Size is less than Need Size , Save only Remain Size");
				...
				{
					SDEC_DTV_SOC_Message( SDEC_SWPARSER, "SWP_FLTSTAT_SEC_HEADER_DONE");
					
					ret = SDEC_SWP_SectionFiltering(&(gSWTSFilter[0].au8HeaderBuffer[0]));
					if(ret != 0)
					{
						SDEC_DTV_SOC_Message( SDEC_SWPARSER, "SDEC_SWP_SectionFiltering Error [%d]",ret);
						gSWTSFilter[0].eFilterState = SWP_FLTSTAT_SEC_HEADER_NEED;
						gSWTSFilter[0].ui8NeedHeaderSize = SECTION_FILTER_LENGTH;
						goto NEXT_TS;
					}
				...
				{
					SDEC_DTV_SOC_Message( SDEC_SWPARSER, "SWP_FLTSTAT_SEC_HEADER_FILTER_DONE");
					
					/* Get Section Length */
					gSWTSFilter[0].uiSectionLength = (((gSWTSFilter[0].au8HeaderBuffer[1] & 0x0F) << 8) | gSWTSFilter[0].au8HeaderBuffer[2]);
					SDEC_DTV_SOC_Message( SDEC_SWPARSER, "Section Header Filtering Done SecLen[%d]",gSWTSFilter[0].uiSectionLength);
					/* Copy Section Header */
					...
					SDEC_DTV_SOC_Message( SDEC_SWPARSER, "Header Copy Done, remain Section Data [%d]",gSWTSFilter[0].uiRemainSectionData);
					/* Copy Remain Payload */
					...
					SDEC_DTV_SOC_Message( SDEC_SWPARSER, "Cur. SecData in Remain TS Payload Copy Done, Final Remain SecData [%d]",gSWTSFilter[0].uiRemainSectionData);
					
					if(gSWTSFilter[0].uiRemainSectionData == 0)
					{
						gSWTSFilter[0].eFilterState = SWP_FLTSTAT_SEC_ALL_COPY_DONE;
						SDEC_DTV_SOC_Message( SDEC_SWPARSER, "SWP_FLTSTAT_SEC_ALL_COPY_DONE 1 ui8RemainSize[%d]",ui8RemainSize);
					}
					else
					{
				...
				{
					SDEC_DTV_SOC_Message( SDEC_SWPARSER, "SWP_FLTSTAT_SEC_HEADER_COPY_DONE");
					...
					if(ui8RemainSize >= gSWTSFilter[0].uiRemainSectionData)
					{
						...
						gSWTSFilter[0].uiRemainSectionData = 0;
						gSWTSFilter[0].eFilterState = SWP_FLTSTAT_SEC_ALL_COPY_DONE;
						SDEC_DTV_SOC_Message( SDEC_SWPARSER, "SWP_FLTSTAT_SEC_ALL_COPY_DONE 2 ui8RemainSize[%d]",ui8RemainSize);
					}
					else
					{
						LX_MEMCPY_CIR_TO_CIR(gSWTSFilter[0].pui8WritePtr, pui8Data, ui8RemainSize, gSWTSFilter[0].pui8BufStart, gSWTSFilter[0].pui8BufEnd, gSWTSFilter[0].pui8DNBufStart, gSWTSFilter[0].pui8DNBufEnd );
						...
						gSWTSFilter[0].uiRemainSectionData -= ui8RemainSize;
						ui8RemainSize = 0;
						SDEC_DTV_SOC_Message( SDEC_SWPARSER, "Copy Section Body gSWTSFilter[0].uiRemainSectionData[%d] ui8RemainSize[%d]",gSWTSFilter[0].uiRemainSectionData,ui8RemainSize);
				...
				if(gSWTSFilter[0].eFilterState == SWP_FLTSTAT_SEC_ALL_COPY_DONE)
				{
					...
					if(ret == 0)
					{
						SDEC_DTV_SOC_Message( SDEC_SWPARSER, "Section Complete Callback!");
						...
						if(continuous_section == 1)
						{
							...
							SDEC_DTV_SOC_Message( SDEC_SWPARSER, "Continue to Parse Continuous Section ui8RemainSize[%d]!!!\n",ui8RemainSize);
							goto parse_continuous_section;
						}
					}
					else
					{
						SDEC_DEBUG_Print("SDEC_SWP_CheckCRC Error [%d]",ret);
						gSWTSFilter[0].eFilterState = SWP_FLTSTAT_SEC_HEADER_NEED;
						gSWTSFilter[0].ui8NeedHeaderSize = SECTION_FILTER_LENGTH;
...
NEXT_TS:
			...
			SDEC_DTV_SOC_Message( SDEC_SWPARSER, "NOW NEXT_TS Remain PVR Data [%d]",uiSize);
		}

		/* Update Read Pointer - 필요 없을 듯 Check 해서 삭제 요 */
		if( RET_OK != PVR_DD_SetDownloadReadAddr(pvr_ch, (UINT32)SWP_DN_VIRT_TO_PHYS(pui8DNBuffer)))
		{
			SDEC_DEBUG_Print("KADP_PVR_DnGetWriteAddr Error");
			continue;
		}
	...
	SDEC_DEBUG_Print("Stop SDEC SW Parser thread\n");
...
void SDEC_RFOUT_ComponentInit(S_SDEC_PARAM_T *stpSdecParam)
{
	...
	if(_gRFoutDnVirtAddr != 0)
	{
		...
		SDEC_DTV_SOC_Message( SDEC_RFOUT, "iounmapped previous ioremap pointer");
	}
	...
	SDEC_DTV_SOC_Message( SDEC_RFOUT, "RF Out DN_BUF [0x%x] size [0x%x]", baseAddr, size);		
	SDEC_DTV_SOC_Message( SDEC_RFOUT, "virtAddr[0x%x]", _gRFoutDnVirtAddr);
	SDEC_DTV_SOC_Message( SDEC_RFOUT, "_gSecCirVirtOffset[0x%x]", _gRFoutDnVirtOffset);
	
	gVa_DNBuf.pStart = gVa_DNBuf.pRead = gVa_DNBuf.pWrite = (UINT8 *)_gRFoutDnVirtAddr;
	gVa_DNBuf.pEnd = (UINT8*)(gVa_DNBuf.pStart + size);

	SDEC_DTV_SOC_Message( SDEC_RFOUT, "gVa_DNBuf.pStart[0x%x]", (UINT32)gVa_DNBuf.pStart);
	SDEC_DTV_SOC_Message( SDEC_RFOUT, "gVa_DNBuf.pRead[0x%x]", (UINT32)gVa_DNBuf.pRead);
	SDEC_DTV_SOC_Message( SDEC_RFOUT, "gVa_DNBuf.pWrite[0x%x]", (UINT32)gVa_DNBuf.pWrite);
	SDEC_DTV_SOC_Message( SDEC_RFOUT, "gVa_DNBuf.pEnd[0x%x]", (UINT32)gVa_DNBuf.pEnd);
...
//	UINT8 ch = stpSdecParam->ui8RFBypassChannel;
	...
	SDEC_DTV_SOC_Message( SDEC_TRACE, "<--SDEC_RFOUT_SetSectionBuffer");
	...
	SDEC_DTV_SOC_Message( SDEC_RFOUT, "baseAddr[0x%x]", baseAddr);
	SDEC_DTV_SOC_Message( SDEC_RFOUT, "size[0x%x]", size);
	...
	SDEC_DTV_SOC_Message( SDEC_RFOUT, "virtAddr[0x%x]", _gRFoutSecCirVirtAddr);
	SDEC_DTV_SOC_Message( SDEC_RFOUT, "_gSecCirVirtOffset[0x%x]", _gRFoutSecCirVirtOffset);
	...
	SDEC_DTV_SOC_Message( SDEC_RFOUT, "gVa_SecBuf.pStart[0x%x]", (UINT32)gVa_SecBuf.pStart);
	SDEC_DTV_SOC_Message( SDEC_RFOUT, "gVa_SecBuf.pRead[0x%x]", (UINT32)gVa_SecBuf.pRead);
	SDEC_DTV_SOC_Message( SDEC_RFOUT, "gVa_SecBuf.pWrite[0x%x]", (UINT32)gVa_SecBuf.pWrite);
	SDEC_DTV_SOC_Message( SDEC_RFOUT, "gVa_SecBuf.pEnd[0x%x]", (UINT32)gVa_SecBuf.pEnd);
...
static int _SDEC_RFOUT_Thread(void *data)
{
	...
	while(!kthread_should_stop())
	{
		...
		if ( ret == RET_OK )
		{
			...
			if( ui32ReceivedEvent != (UINT32)eWaitEvent )
			{
				SDEC_DEBUG_Print("SDEC RFOUT OS_RecvEvent Error Wait[%d] Received[%d]",(UINT32)eWaitEvent, ui32ReceivedEvent );
				continue;
			}
			else
			{
				...
				{
					SDEC_DTV_SOC_Message( SDEC_RFOUT, "SDEC RFOUT Thread Received Start \n");
					...
					uiPhyDNWritePtr = astDvrMemMap[ch].stDvrDnBuff.ui32WritePtr;
					uiPhyDNReadPtr =  astDvrMemMap[ch].stDvrDnBuff.ui32ReadPtr;
					uiPhyDNBasePtr = astDvrMemMap[ch].stDvrDnBuff.ui32BufferBase;
					uiPhyDNEndPtr = astDvrMemMap[ch].stDvrDnBuff.ui32BufferEnd;
	...
	//				uiPhySECEndPtr =  stpSdecParam->stSdecMeminfo[LX_SDEC_CH_A][1].ui32Endptr;
		...
		if( RET_OK != PVR_DD_GetDownloadWriteAddr(ch, &uiPhyDNWritePtr))
		{
			SDEC_DEBUG_Print("KADP_PVR_DnGetWriteAddr Error");
			continue;
		}
		...
		SDEC_DTV_SOC_Message(SDEC_RFOUT, "DN w[0x%x]r[0x%x]data_len[0x%x] ", (UINT32)(gVa_DNBuf.pWrite), (UINT32)(gVa_DNBuf.pRead), data_len );
		...
		if(data_len > SDEC_RFOUT_THRESHOLD)
		{
			...
			for(i=0;i<packet_cnt;i++)
			{
				...
				{ 
					SDEC_DTV_SOC_Message(SDEC_RFOUT, "SYNC ERROR [0x%x] ", gVa_DNBuf.pRead[4]);
	...
	SDEC_DEBUG_Print("Stop SDEC RFOUT thread\n");
...
void SDEC_RFOUT_Init(S_SDEC_PARAM_T *stpSdecParam)
{
	...
	if(stRFOUT_Thread)
	{
		...
		SDEC_DTV_SOC_Message(SDEC_NOTI, "Run SDEC RFOUT Thread PID:[%d]\n", stRFOUT_Thread->pid);
	}
	else
	{
		SDEC_DTV_SOC_Message(SDEC_NOTI, "SDEC RFOUT Thread Already Created\n");
	}
...
DTV_STATUS_T SDEC_RFOUT_Enable
	(S_SDEC_PARAM_T *stpSdecParam,
	LX_SDEC_RFOUT_ENABLE_T *stpLXSdecRFOUTEnable)
{
	...
	LX_SDEC_CHECK_CODE( stpSdecParam == NULL, return INVALID_PARAMS, "Invalid parameters" );
	LX_SDEC_CHECK_CODE( stpLXSdecRFOUTEnable == NULL, return INVALID_ARGS, "Invalid argument" );

	OS_LockMutex(&stpSdecParam->stSdecMutex);

	SDEC_DTV_SOC_Message( SDEC_TRACE, "<--SDEC_RFOUT_Enable");
	...
	if(bEnable)
	{
		SDEC_DTV_SOC_Message( SDEC_RFOUT, "Send START to SDEC RFOUT Thread\n");
	...
	{
		SDEC_DTV_SOC_Message( SDEC_RFOUT, "Send STOP to SDEC RFOUT Thread\n");		

...
static int CLK_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	CLK_KDRV_LOG( CLK_TRACE, "[%s] Memorizing reg content\n", CLK_MODULE);
	...
	CLK_KDRV_LOG( CLK_TRACE, "[%s] done suspend\n", CLK_MODULE);
...
static int CLK_resume(struct platform_device *pdev)
{
	...
	CLK_KDRV_LOG( CLK_TRACE, "[%s] Restoring reg content\n", CLK_MODULE);
	...
	CLK_KDRV_LOG( CLK_TRACE, "[%s] done resume\n", CLK_MODULE);
	return 0;
}
/**
 *

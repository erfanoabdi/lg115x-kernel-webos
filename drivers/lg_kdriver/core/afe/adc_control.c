...
int ADC_Periodic_Task(void)
{
	...
	if(1)
	{
		...
		else
		{
			...
			if( ( gADC_type == LX_ADC_INPUT_SOURCE_YPBPR) && ( ADC_Check_Sync_Exist() > 0) )
			{
				if((gResolution == 59) || (gResolution == 61))
				{
					sync_level_count ++;
					sync_level = ADC_Read_Sync_Low_Level();

					if(sync_level > sync_level_max)
					{
					//	ADC_DEBUG("### Sync Level [0x%x], Max Sync Level [0x%x]###\n", sync_level, sync_level_max);
						sync_level_max = sync_level;
					}
					...
					if(sync_level_count > 10)
					{
						...
						sync_level = sync_level_max;
						sync_level_max = 0;
						if( ( sync_level < g_comp_sync_level_108 ) && (prev_sync_level == 100) && (sync_level != 0) )
						{
						...
						else if( ( sync_level > g_comp_sync_level_100 ) && (prev_sync_level == 108))
						{

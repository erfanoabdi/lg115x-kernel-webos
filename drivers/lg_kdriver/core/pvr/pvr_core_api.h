...
typedef struct  tagPieDataTable { // 0x2780
	UINT32	ui32BufNum;
	UINT32	ui32Val;
} PIE_DATA_T;

/* PIE database */
typedef enum __PVR_INDEX_TYPE_T
{
	PVR_IDX_SEQ 	= 0x01,
	PVR_IDX_I_PIC	= 0x02,
	PVR_IDX_SEQ_I_PIC	= 0x03,
	PVR_IDX_P_PIC	= 0x04,
	PVR_IDX_B_PIC	= 0x08,
	PVR_IDX_NONE	= 0xFF
}PVR_KDRV_INDEX_TYPE_T;

typedef struct __PVR_KDRV_INDEX_T
{
	PVR_KDRV_INDEX_TYPE_T	indexType;
	UINT32					byteOffset;
}PVR_KDRV_INDEX_T;

#define PIE_MAX_ENTRIES_LOCAL	1024/* 256 */
#define PIE_MAX_DATA_LOCAL		64

typedef struct tagDvrModStruct
{
	UINT32			ui32Channel;	//Ch_A/Ch_B
	//Present in DVR Memmap structure
#if 0
	DVR_BUFF_STRUCT	stDvrDnBuff;
	DVR_BUFF_STRUCT	stDvrUpBuff;
#endif /* #if 0 */
#if 0
#ifdef DVR_USE_EVENT_FOR_SIGNALS
	wait_queue_head_t	wqDVR_DownEvent;
	wait_queue_head_t	wqDVR_UpEvent;
#else
	struct semaphore semDvrDnSignal;
	struct semaphore semDvrUpSignal;
#endif /* DVR_USE_EVENT_FOR_SIGNALS */
#endif /* #if 0 */
#if 1
	...
	UINT8		ui8DownloadFirstTime;
	UINT8		ui8DownloadMode;	//0-Normal, 1-timeshift, 2-simulation mode
	UINT8		ui8AbortUpload;
	UINT8		ui8AbortDownload;
	UINT32		ui32PrevBufNum;
	UINT32		ui32CurrBufNum;
	UINT32		ui32TotalBufCounter;
	UINT8		ui8UploadMode;	//0-Normal, 1-timeshift, 2-simulation mode, 3-nch mode
	UINT8		ui8UpSignalStatus;	//B0-UL_OVERLAP, B1-SDEC_EPT
	UINT8		ui8UpSpaceAvailable;	//B0-UL_OVERLAP, B1-SDEC_EPT
}DVR_MOD_T;

typedef struct tagDvrPieDataStruct
{
	UINT32		ui32Channel;	//Ch_A/Ch_B
	PIE_DATA_T 	astPieData[PIE_MAX_DATA_LOCAL];
	UINT32		ui32PieWrIndex;
}DVR_PIE_DATA_T;
...
typedef struct tagPvrCurrentBufferState
{
	LX_PVR_BUF_STAT_T	eDnBufState;
	LX_PVR_BUF_STAT_T	eUpBufState;
	LX_PVR_BUF_STAT_T	ePieBufState;
}DVR_BUFFER_STATE_T;

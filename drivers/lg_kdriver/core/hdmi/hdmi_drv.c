...
static int HDMI_suspend(struct platform_device *pdev, pm_message_t state)
{
  ...
  {
    printk("[%s]{Hibernation] HDMI_Open is not done.. skip suspend\n",HDMI_MODULE);  
    return 0;
  }
  //LGE_UPDATE_E shinhoe.kim@lge.com
...
static int HDMI_resume(struct platform_device *pdev)
{
  ...
  if(g_HDMI_Open_done == false)
  {
    printk("[%s]{Hibernation] HDMI_Open is not done.. skip resume\n",HDMI_MODULE);  
    return 0;
  }
  //LGE_UPDATE_E shinhoe.kim@lge.com
...
static long HDMI_Ioctl ( struct file *filp, unsigned int cmd, unsigned long arg )
{
	...
	switch(cmd)
	{
		...
		case HDMI_IOR_GET_COPY_INFO:
			{
				LX_HDMI_AUDIO_COPY_T *pCopyInfo;

				pCopyInfo = (LX_HDMI_AUDIO_COPY_T *)kmalloc(sizeof(LX_HDMI_AUDIO_COPY_T), GFP_KERNEL);

				ret = HDMI_GetAudioCopyInfo(pCopyInfo);
				ret |= copy_to_user((void *)arg, (void *)pCopyInfo, sizeof(LX_HDMI_AUDIO_COPY_T));

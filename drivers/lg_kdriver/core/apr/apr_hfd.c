...
#define FD3MSG	if((g_msg_cnt==8)&&(pCfg->fd3.enb == LX_APR_FD3_FMT))printk
#define FD3DBG	if((g_msg_cnt==8)&&(pCfg->fd3.enb == LX_APR_FD3_LOG))printk
#define FD3ALG  if((g_msg_cnt==8)&&(pCfg->fd3.enb == LX_APR_FD3_LOG))printk	
#define FD3PRT  if(pCfg->fd3.enb == LX_APR_FD3_LOG)printk
...
int APR_FD3_Detect3D(void)
{
	...
	FD3DBG("Finish]]] FD3 result : %d (0:2D,4:SS,5:TB)\n\n", m2_fmt);

...
## calculate with decision rule
{
	...
	if(scene_type == 1)
	{
		...
		else
		{
			...
			if((result == LX_APR_FMT_3D_SS) && (tbr > 20))
			{
				result = LX_APR_FMT_UNKNOWN;
			}
			else if((result == LX_APR_FMT_3D_TB) && (ssr > 20))
			{
				result = LX_APR_FMT_UNKNOWN;
			}
		}
...
# 20130115 added for exception (2D fail related)
			if(detect_format == LX_APR_FMT_3D_SS) {
				seg_max_sum[0] = 0;
				seg_max_sum[1] = 0;
				seg_max_cnt2 = 0;
				...
				if ((seg_max_cnt2 == 4) || (seg_max_cnt3 == 4) )
				{
					...
					tmp_b_sum[0] = (tmp_b_sum_th0*img_width*img_height) / 2048;
					tmp_b_sum[1] = (tmp_b_sum_th1*img_width*img_height) / 2048;
		...
		# Exception for CG case with data of Method 1 
		if(detect_format == LX_APR_FMT_3D_SS)
		{
			if((tbr > 20) || (tbr > ssr))
			{
				detect_format = LX_APR_FMT_UNKNOWN;
				#printf("###  [%d] exception 1 (tbr[%d] > ssr[%d]) ###\n", NR, tbr, ssr);
			}
		}
		else if(detect_format == LX_APR_FMT_3D_TB)
		{
			...
			{
				detect_format = LX_APR_FMT_UNKNOWN;
				#printf("###  [%d] exception 2 (ssr[%d] > tbr[%d]) ###\n", NR, ssr, tbr);
			}
		}
		if(1){
		if(detect_format == LX_APR_FMT_3D_TB)
...
#		printf("      lrc_lr_diff[%d] lrc_tb_diff[%d] thr7[%d] ssr[%d] tbr[%d]\n", lrc_lr_diff, lrc_tb_diff, thr7, ssr, tbr);
	...
	fmt_before_vote[voting_count] = result;
	apr_voted_fmt[voting_count]   = apr_fmt;
	voting_count++;
}
...
# voting start
	...
	vld_thr        = 5;
	valid_flag     = 0;
	APR_VOTE_STACK_SIZE_ON_FORMAT_CHANGE = 4;
	for(vc=0;vc<(voting_count - 1);vc++)
	{
		cur_v_fmt = fmt_before_vote[vc];
		for(i=0;i<LX_APR_FMT_MAX;i++) vote_nominee[i] = 0;
		for(i=0;i<vld_thr-1;i++)
		{
			voting_buf[vld_thr - 1 - i] = voting_buf[vld_thr - 2 - i];
			temp = voting_buf[vld_thr - 1 - i];
			if((i<(APR_VOTE_STACK_SIZE_ON_FORMAT_CHANGE-1))&&(temp < LX_APR_FMT_MAX)) vote_nominee[temp] += 1;
		}
		voting_buf[0] = cur_v_fmt;
		temp = cur_v_fmt;
		vote_nominee[temp] += 1;
		for(i=0;i<LX_APR_FMT_MAX;i++)
		{
			if((i != LX_APR_FMT_UNKNOWN)&&(vote_nominee[i] >= (APR_VOTE_STACK_SIZE_ON_FORMAT_CHANGE - 1)))
			{
				valid_flag = 1;
				cur_v_fmt = i;
			}
		...
		if(valid_flag)
			fmt_after_vote = cur_v_fmt;

		if(fmt_after_vote != apr_voted_fmt[vc])
		{
#			printf("  mismatch vote[%d] - %d : %d\n", vc, fmt_after_vote, apr_voted_fmt[vc]);
		}			
	}
# voting end
...
END {
	...
	printf "  3D : %f %%   (%d / %d)\n\n", (match_ss+match_tb)*100/(total_ss+total_tb), (match_ss + match_tb), (total_ss + total_tb)

...
static int APR_CheckConfig(void)
{
	...
	static UINT32 cur_scan_info    = 0;
	static UINT32 cur_color_sample = 0;
	...
	do {
		if(cur_src_type != pCfg->src_type)
		{
			...
			if(_g_apr_trace >= LX_APR_CMD_LOG)
		...
		if((cur_scan_info != pCfg->fd3.scan) || (cur_color_sample != pCfg->fd3.color) || (cur_input_height != pCfg->in_win_y))
		{
...
static int APR_GetLrcrHistogram(void)
{
	...
	do {
		...
		{
			if(pCfg->hst_enb == LX_APR_HST_RAW_DATA)
			{
...
#if 0 // remain for test
		if((hist_info.src.set_cnt[0] == (hist_info.src.set_cnt[1])) \
				&& (prev_src_count[1] < hist_info.src.set_cnt[1]))
		{
			if(pCfg->hst_enb == LX_APR_HST_RAW_DATA)
			{
				APR_MSG("### SRC Histogram difference\n");
				APR_MSG("tb  [%08d] ss  [%08d]\n", hist_info.src.dif_max[0], hist_info.src.dif_max[1]);
				APR_MSG("pxl0[%08d] pxl1[%08d]\n", hist_info.src.pxl_cnt[0], hist_info.src.pxl_cnt[1]);
				APR_MSG("cnt0[%08d] cnt1[%08d]\n", hist_info.src.set_cnt[0], hist_info.src.set_cnt[1]);
			}
			prev_src_count[0] = hist_info.src.set_cnt[0];
			prev_src_count[1] = hist_info.src.set_cnt[1];
			if(hist_info.src.set_cnt[0] == 255)
			{
				prev_src_count[0] = 0;
				prev_src_count[1] = 0;
			}
			...
			for(i=0;i<9;i++) {
				pData->hist_src.tb_diff[i] = hist_info.src.dif_max[i<<1];
				pData->hist_src.lr_diff[i] = hist_info.src.dif_max[(i<<1)+1];
				pData->hist_src.tb_ratio[i] = hist_info.src.pxl_cnt[i<<1];
				pData->hist_src.ss_ratio[i] = hist_info.src.pxl_cnt[(i<<1)+1];
			}
			pData->src_complete_flag = TRUE; 
		}
#endif
...
static int APR_NegotiateFormat(BOOLEAN nego_enble)
{
	...
	do {
		...
		if((pCfg->out_type == LX_PE_OUT_2D) || (pCfg->out_type == LX_PE_OUT_2D_3D) || (pCfg->out_type == LX_PE_OUT_PIP))
		{
			...
			if(0)//pData->tpd < 30)
			{
				if(pCfg->cap.enb == LX_APR_CAP_LOG){APR_MSG("case 0 -  tpd[%d]  no nego\n", pData->tpd);}
				pCfg->apr_fmt = pCfg->fmt_after_vote[APR_METHOD_3];
				break;
			}
			...
			if((pCfg->fmt_after_vote[APR_METHOD_3] == LX_APR_FMT_3D_SS) || (pCfg->fmt_after_vote[APR_METHOD_3] == LX_APR_FMT_3D_TB))
			{
				...
				if(0)//unk_count == pThr->vld_thr)
				{
					pCfg->apr_fmt = LX_APR_FMT_UNKNOWN;
					if(pCfg->cap.enb == LX_APR_CAP_LOG){APR_MSG("case 3 \n");}
					break;
				}
			}		
			pCfg->apr_fmt = pCfg->fmt_after_vote[APR_METHOD_3];
#else
			if(pCfg->fmt_after_vote[APR_METHOD_3] == LX_APR_FMT_2D_2D)
			{
				if((pData->tpd > 0) && (pData->tpd < 250/*pThr->tpd_thr*/))
				{
					if(pCfg->fmt_after_vote[APR_METHOD_2] == LX_APR_FMT_3D_SS)
						pCfg->apr_fmt = LX_APR_FMT_3D_SS;
					else if(pCfg->fmt_after_vote[APR_METHOD_2] == LX_APR_FMT_3D_TB)
						pCfg->apr_fmt = LX_APR_FMT_3D_TB;
					else 
						pCfg->apr_fmt = pCfg->fmt_after_vote[APR_METHOD_3];
				}
				else
...
static int APR_DumpCurrentConfig(void)
{
	...
	do {
		APR_MSG("\n");
		APR_MSG("src_type    [%d] (0:DTV,1:ATV,2:CVBS,3:COMP,4:PC,5:HDMI)\n", pCfg->src_type);
		APR_MSG("out_type    [%d] (0:2D,1:3D-2D,2:2D-3D,3:UD,4:TB,5:SS,6:FS,7DUAL_TB,8:DUAL_SS,9:DUAL_FULL,10:PIP)\n", pCfg->out_type);
		APR_MSG("in_type     [%d] (0:TB,1:SS,2:QC,3:CB,4:FP,5:FPI,6:FA,7:FS,8:LA,9:SSF,10:DUAL,11:CA,12:LAH)\n", pCfg->in_type);
		APR_MSG("Image class [%d] (0:TP,1:2D_HARD,2:2D_WEAK_SS,3:2D_WEAK_TB,4:SS_HARD,5:SS_WEAK,6:TB_HARD,7:TB_WEAK)\n", pCfg->img_class);
		APR_MSG("Enable     : apr[%d] hst[%d] cap[%d] fd3[%d]\n", pCfg->enable, pCfg->hst_enb, pCfg->cap.enb, pCfg->fd3.enb);
		APR_MSG("Detect fmt : src_hst[%d],lrc_hst[%d],ldf[%d],fd3[%d],apr[%d] (0:2D,4:SS,5:TB,7:UN)\n", pCfg->cur_src_fmt,pCfg->cur_lrc_fmt,pCfg->ldf_fmt,pCfg->fd3_fmt,pCfg->apr_fmt);
		APR_MSG("input  win : w[%d] h[%d]\n", pCfg->in_win_x, pCfg->in_win_y);
		APR_MSG("output win : w[%d] h[%d]\n", pCfg->out_win_x, pCfg->out_win_y);
		APR_MSG("scan       : [%d] (0:I,1:P)\n", pCfg->scan);
		APR_MSG("\n");
		APR_MSG("\n");
	} while(0);
...
static int APR_DecideFormatBySRCHST(void)
{
	...
	APR_DTA_T *pData= &apr_info.dta;
	APR_THR_T *pThr = &apr_info.thr;
	LX_APR_FORMAT tmp_fmt = LX_APR_FMT_2D_2D;
	UINT32 match_count = 0;
	UINT32 no_match_count = 0;
	UINT32 diff_tb_y;
	UINT32 diff_ss_y;
	UINT32 diff_tb_h;
	UINT32 diff_ss_h;
	...
	UINT32 src_thr_0 = 300000;



	do {
		/* [0,1]y tb,ss,[2,3]cb,[4,5]cr, [6,7]g,[8,9]b,[10,11]r, [12,13]h,[14,15]s,[16,17]v */
		diff_tb_y = pData->hist_src.tb_diff[0];
		diff_ss_y = pData->hist_src.lr_diff[1];
		diff_tb_h = pData->hist_src.tb_diff[6];
		diff_ss_h = pData->hist_src.lr_diff[6];
		diff_tb_s = pData->hist_src.tb_diff[7];
		diff_ss_s = pData->hist_src.lr_diff[7];
		tpd       = pData->tpd;
		delta_ss  = pData->ssr;
		delta_tb  = pData->tbr;

		if((diff_tb_y > src_thr_0) && (diff_ss_y > src_thr_0))
			no_match_count++;
		if((diff_tb_h > src_thr_0) && (diff_ss_h > src_thr_0))
			no_match_count++;
		if((diff_tb_s > src_thr_0) && (diff_ss_s > src_thr_0))
			no_match_count++;
...
//		if(pCfg->apr_fmt == prev_fmt) break;
		...
		ret = APR_OSD_PrintParam(	"  tpd : ", pData->tpd,             \
									...
									NULL, 0,                            \
								   	0, line_idx++);
		ret = APR_OSD_PrintParam(	"  mtn : ", pData->motion_l,        \
									NULL, 0,                            \
									NULL, 0,                            \
									NULL, 0,                            \
								   	0, line_idx++);
		ret = APR_OSD_PrintParam(	"  ssr : ", pData->ssr,             \
									NULL, 0,                            \
									NULL, 0,                            \
									NULL, 0,                            \
								   	0, line_idx++);
		ret = APR_OSD_PrintParam(	"  tbr : ", pData->tbr,             \
									NULL, 0,                            \
									NULL, 0,                            \
									NULL, 0,                            \
								   	0, line_idx++);
		ret = APR_OSD_PrintParam(	"  l-r : ", pData->hist_lrc.lr_diff,         \
									NULL, 0,                            \
									NULL, 0,                            \
									NULL, 0,                            \
								   	0, line_idx++);
		ret = APR_OSD_PrintParam(	"  t-b : ", pData->hist_lrc.tb_diff,         \
									NULL, 0,                            \
									NULL, 0,                            \
									NULL, 0,                            \
								   	0, line_idx++);
		memset(str, 0, STRING_MAX_SIZE);
		strcat(str, "Capture info ");
		strcat(str, (pCfg->out_type==LX_PE_OUT_2D)?"(from sub-scaler)":"(from L/R scaler)");
		ret = APR_OSD_DrawString(	str, 0, line_idx++);
		ret = APR_OSD_PrintParamHex(	"  frame address:", capture_info.frame_base_phys[0], \
									" ", capture_info.frame_base_phys[1],\
									NULL, 0,                            \
									NULL, 0,                            \
								   	0, line_idx++);
		ret = APR_OSD_PrintParam(	"  width  : ", capture_info.h_size, \
									NULL, 0,                            \
									NULL, 0,                            \
									NULL, 0,                            \
								   	0, line_idx++);
		ret = APR_OSD_PrintParam(	"  height : ", capture_info.v_size, \
									NULL, 0,                            \
									NULL, 0,                            \
									NULL, 0,                            \
								   	0, line_idx++);
		ret = APR_OSD_PrintParam(	"  stride : ", capture_info.stride, \
									NULL, 0,                            \
									NULL, 0,                            \
									NULL, 0,                            \
								   	0, line_idx++);
		ret = APR_OSD_PrintParam(	"  offset : ", capture_info.top_offset, \
									NULL, 0,                            \
									NULL, 0,                            \
									NULL, 0,                            \
								   	0, line_idx++);
		ret = APR_OSD_PrintParam(	"  h-line : ", capture_info.h_line_num,\
									NULL, 0,                            \
									NULL, 0,                            \
									NULL, 0,                            \
								   	0, line_idx++);
		ret = APR_OSD_PrintParam(	"  c-line : ", (pCfg->out_type==LX_PE_OUT_2D)?APR_MAX_CENTER_LINE:0, \
									NULL, 0,                            \
									NULL, 0,                            \
									NULL, 0,                            \
								   	0, line_idx++);
		ret = APR_OSD_PrintParam(	"  s-crop : ", capture_info.lr_crop,\
									NULL, 0,                            \
									NULL, 0,                            \
									NULL, 0,                            \
								   	0, line_idx++);

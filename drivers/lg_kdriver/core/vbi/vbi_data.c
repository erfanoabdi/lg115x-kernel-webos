...
int VBI_CC_PutVBIData(unsigned int *data , unsigned int size)
{
	...
	/* 방어 코드 삽입 */
	if(size >= 2) VBI_PRINT("VBI CC : Warning! VBI_CC_PutVBIData function cannot operate on mutliple data size over 1.\n");
...
int VBI_PutCGMSData(sCGMS *data)
{
	...
	{
		VBI_PRINT("Error! Memory for CGMS data is not allocated!\n");
		return RET_ERROR;
	}
	else
	{
...
int VBI_CC_GetVBIData(LX_CC_DATA_T *cc_data , unsigned int size)
{
	...
	if(gVBI_data_mem_addr == 0) {
		VBI_PRINT("Data mem addr is NULL!\n");
		return RET_ERROR;
	}

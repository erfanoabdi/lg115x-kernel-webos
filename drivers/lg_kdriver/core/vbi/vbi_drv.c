...
static int VBI_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	//VBI_CC_RunSuspend(); // won.hur

	drv_data->is_suspended = 1;
	VBI_PRINT("[%s] done suspend\n", VBI_MODULE);
#endif
	return 0;
}
...
int VBI_Init(void)
{
	...
	OS_DEBUG_EnableModuleByIndex ( g_vbi_debug_fd, 0, DBG_COLOR_NONE );
	OS_DEBUG_EnableModuleByIndex ( g_vbi_debug_fd, 1, DBG_COLOR_NONE );
	OS_DEBUG_EnableModuleByIndex ( g_vbi_debug_fd, 2, DBG_COLOR_NONE );
	OS_DEBUG_EnableModuleByIndex ( g_vbi_debug_fd, 3, DBG_COLOR_NONE );
	...
	{
		VBI_PRINT("[%s] platform driver register failed\n",VBI_MODULE);
	...
	else
	{
		if(platform_device_register(&vbi_device))
		{
			...
			VBI_PRINT("[%s] platform device register failed\n",VBI_MODULE);
		}
		else
		{
	...
	if ( err < 0 )
	{
		DBG_PRINT_ERROR("can't register vbi device\n" );
		return -EIO;
	}
	...
	for ( i=0; i<VBI_MAX_DEVICE; i++ )
	{
		...
		if (err)
		{
			DBG_PRINT_ERROR("error (%d) while adding vbi device (%d.%d)\n", err, MAJOR(dev), MINOR(dev) );
			return -EIO;
		}
...
static int VBI_MMap(struct file *file, struct vm_area_struct *vma)
{
	...
	{
	 	VBI_PRINT("ERROR : GFX_SurfaceMmap leng is over\n");
	 	VBI_PRINT("start : %lx end : %lx offset : %lx len : %lx\n" ,

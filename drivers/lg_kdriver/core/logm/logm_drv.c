...
#define error(fmt, arg...)	do{printk (KERN_ERR "[ERROR]%s.%d: " fmt, __func__, __LINE__, ##arg);}while(0)
									 ...
									 unsigned int cmd)
{
	...
	else if(m.ctrl == LOGM_MASK_CTRL_ALL_WR)
	{
		int i;
		unsigned long flag;
		LX_LOGM_LOGOBJ_T *map = g_logm_obj_map;
	...
	else if(m.ctrl == LOGM_MASK_CTRL_BIT_ENABLE)
	{
		ret = LOGM_ObjBitMaskEnable(m.fd, m.bit);
		LOGM_CHECK_ERROR(ret != 0, return RET_ERROR,
			"fail obj mask bit enable\n");
	}
	else if(m.ctrl == LOGM_MASK_CTRL_BIT_DISABLE)
	{
		...
		char __user *data, size_t size, loff_t *off)
{
	...
	do
	{
		ret = LOGM_ReadLog(bc, data, readed, size);
		if (ret == -ENOMEM)
			break;

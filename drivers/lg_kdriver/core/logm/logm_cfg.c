		...
		LX_LOGM_LOGLEVEL_T level, const char *str, int len)
{
	...
	if(!g_logm_short_format)
	{
		printk (KERN_ERR LOGM_NAME_LEVEL_FORMAT " (%04d) %s\n",
				obj->name,
 ...
 * TODO: call in lock
 * @param *bc       [in/out] variables of buffer control
 * @param *data     [out] user buffer to store a log
 * @param readed    [in]
 * @param size      [in]
 * @return int
 *
...
int LOGM_ReadLog(struct logm_buffer_ctrl *bc, 
		char __user *data, int readed, int size)
{
	int			ret;
	int32_t		buffer_tail_tmp;
	char		*buf = data+readed;//__user
	int32_t		len = size-readed;
	struct		logm_header *h;

	//TODO: memcpy => copy_to_user
...
int LOGM_ParseLog(struct logm_header *h, char *dest, int32_t dest_len)
{
	...
	if(!g_logm_short_format)//normal log
		hlen = 5+6+12+12;//string length of header : "[%5lu.%06lu] " LOGM_NAME_LEVEL_FORMAT " (%04d) "
	else
		hlen = 5+6+12+5;
	...
	if(!g_logm_short_format)
	{
		hlen = snprintf (dest, dest_len,
				"[%5lu.%06u] " LOGM_NAME_LEVEL_FORMAT " (%04d) ",

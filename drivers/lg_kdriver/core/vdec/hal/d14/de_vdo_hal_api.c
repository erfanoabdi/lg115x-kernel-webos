...
void DE_VDO_HAL_SetSeamless(UINT8 ui8bSeamlessMode);
/*----------------------------------------------------------------------------------------
	Static Variables
----------------------------------------------------------------------------------------*/
...
void DE_VDO_HAL_Update(UINT8 ui8VdoCh, S_DE_IPC_T *pDisBuf, UINT32 ui32SyncField)
{
	...
	//1. udDomain setting
	if(pDisBuf->ui16PicWidth > 2048 || ui8SeamlessMode == 1)
		udDomain = 1;
	else
		udDomain = 0;
	...
	if(!ui8SeamlessMode)
	{
		VCP_CH_PIC_INIT(0) = 0x00000011;			// 1. Vcp init(Not Seamless)
		if(is_high_frq && udDomain == 1) //60p
			VCP_CH_PIC_INIT(1) = 0x00000011;		// 1. Vcp init(Not Seamless)
	}
	else
	...
	if(!ui8SeamlessMode)
	{
		VCP_CH_PIC_INIT(0) = 0x00000000;			// 2. Vcp init off(Not Seamless)
		if(is_high_frq && udDomain == 1) //60p
			VCP_CH_PIC_INIT(1) = 0x00000000;		// 2. Vcp init off(Not Seamless)
	}
	else
...
void DE_VDO_HAL_SetSeamless(UINT8 ui8bSeamlessMode)
{
	ui8SeamlessMode = ui8bSeamlessMode;
	// Seamless Mode : 1
	if (ui8bSeamlessMode)
	...
	log_user1("Seamless Mode Status : %s\n",(TOP_RC_TOPCONFIG == 0x000000C8)?"Seamless Mode":"Not Seamless Mode");
}

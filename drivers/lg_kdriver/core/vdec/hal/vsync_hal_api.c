...
static irqreturn_t _IRQHandler (int num, void *arg)
{
	...
	unsigned int irqstatus2 = 0;
	unsigned int int_clear2 = 0;
	
	irqstatus2 = TOP_HAL_GetVsync2IntrStatus(); 
#endif
...
#if defined (CHIP_NAME_d14)
	TOP_HAL_ClearVsync2IntrMsk(int_clear2);
#endif
...
int VSync_HAL_IRQ_Enable (void)
{

	enable_irq (VDEC_IRQ_NUM_SYNC);
	return 0;

}

int VSync_HAL_IRQ_Disable (void)
{

	disable_irq (VDEC_IRQ_NUM_SYNC);
	return 0;

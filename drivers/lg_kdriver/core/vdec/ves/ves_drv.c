	...
	switch( gsVesDrv[ui8VesCh].State )
	{
		...
		case VES_PLAY :
			...
			if( gsVesDrv[ui8VesCh].Config.eVesSrcCh <= VES_SRC_SDEC2 ) // live
			{
				...
				{
					logm_info(vdec_ves, "[VES%d] AU Detected - Type:%d, PTS:%9u, DTS:%9u, STC:%9u, StcDis:%d, Remained:%8x, Start:0x%x Counted:%d \n"
							,ui8VesCh, sPdecAu.eAuType, sPdecAu.ui32PTS, sPdecAu.ui32DTS,
							(PDEC_HAL_GetStc(gsVesDrv[ui8VesCh].Config.eVesSrcCh)&0xFFFFFFFF),
							sPdecAu.bStccDiscontinuity, VES_CPB_GetRemainedBuffer(ui8VesCh),
							sPdecAu.ui32AuStartAddr, gsVesDrv[ui8VesCh].Status.ui32PushedAui);

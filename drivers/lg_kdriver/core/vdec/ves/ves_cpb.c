...
*/
	...
	// work-around code for hevc decoder
#if 0
	if( gsVesCpb[ui8VesCh].eCodecType == PDEC_CODEC_HEVC )
	{
		logm_noti(vdec_ves,"[CPB%d] check time0, austart:%X, size:%d, modified:%d \n"
				,ui8VesCh, (UINT32)pui8AuStartAddr, ui32AuSize, *pui32AuSizeModified);
		copy_from_user( CheckStart, pui8AuStartAddr, 16 );
		if(CheckStart[0] == 0 && CheckStart[1] == 0 && CheckStart[2] == 0 && CheckStart[3] == 1) {
			logm_noti(vdec_ves,"[CPB%d] start code 0 0 0 1 detected \n", ui8VesCh);
			pui8AuStartAddr++;
			ui32AuSize--;
			*pui32AuSizeModified = ui32AuSize;
		}
		logm_noti(vdec_ves,"[CPB%d] check time1, austart:%X, size:%d, modified:%d \n"
				,ui8VesCh, (UINT32)pui8AuStartAddr, ui32AuSize, *pui32AuSizeModified);
	}
#endif

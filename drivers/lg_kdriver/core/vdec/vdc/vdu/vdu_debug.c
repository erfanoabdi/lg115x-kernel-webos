...
static int		_ShowStatus(struct seq_file* file, void* data)
{
	...
	seq_printf(file, "Map type           : %d\n",
						(UINT32)pstOpenParam->eFrameMapType);
...
static void					_PrintResultInfo(VDU_HANDLE_T hInstance, VDU_RESULT_T* pstResult)
{
	...
	case VDU_NOTI_SUCCESS:
		logm_info(vdec_vdu, "[%d]Success\n", (UINT32)hInstance);
		break;

	case VDU_NOTI_DISPLAY:
		logm_info(vdec_vdu, "[%d]Only display\n", (UINT32)hInstance);
		break;

	case VDU_NOTI_NOT_SUPPORTED:
		logm_noti(vdec_vdu, "[%d]SeqInit Fail\n", (UINT32)hInstance);
		break;

	case VDU_NOTI_DPB_FULL:
		logm_info(vdec_vdu, "[%d]DPB Full\n", (UINT32)hInstance);
		break;

	case VDU_NOTI_DPB_EMPTY:
		logm_info(vdec_vdu, "[%d]DPB Empty\n", (UINT32)hInstance);
		break;

	case VDU_NOTI_CPB_EMPTY:
		logm_info(vdec_vdu, "[%d]CPB Empty\n", (UINT32)hInstance);
		break;

	case VDU_NOTI_HEADER_NOT_FOUND:
		logm_noti(vdec_vdu, "[%d]Header not found\n", (UINT32)hInstance);
		break;

	case VDU_NOTI_DECODING_ERROR:
		logm_error(vdec_vdu, "[%d]Unknown decoding error\n", (UINT32)hInstance);
		break;

	case VDU_NOTI_SUSPEND:
		logm_info(vdec_vdu, "[%d]supend decoding\n", (UINT32)hInstance);
		break;

	case VDU_NOTI_FRAME_SKIP:
		logm_info(vdec_vdu, "[%d]Frame skip\n", (UINT32)hInstance);
		break;

	case VDU_NOTI_SKIP_EMPTY:
		logm_info(vdec_vdu, "[%d]Frame skip and CPB Empty\n", (UINT32)hInstance);
		break;

	case VDU_NOTI_REQUEST_AU:
		logm_info(vdec_vdu, "[%d]Request Au\n", (UINT32)hInstance);
		break;

	case VDU_NOTI_END_OF_STREAM:
		logm_info(vdec_vdu, "[%d]End of stream\n", (UINT32)hInstance);
		break;

	default:
		logm_error(vdec_vdu, "[%d]Unknown notiType %d\n", (UINT32)hInstance, (SINT32)(pstResult->eNotiType));
		break;
	}

	logm_info(vdec_vdu, "[%d]Display : %d\n", (UINT32)hInstance, pstResult->si8FrmIndex);
}
...
static void					_PrintFrameInfo(VDU_HANDLE_T hInstance, VDU_FRM_INFO_T* pstFrameInfo)
{
	logm_debug(vdec_vdu, "[%d]%4d(%4d)*%4d|%2d/%2d/%2d/%2d|%2d/%3d/%3d|%d|%4d/%4d(%d)\n", (UINT32)hInstance,
	...
	logm_debug(vdec_vdu, "%08X|%08X/%08X/%08X|%2d-%2d|%08X(%08X)|%X\n",
							pstFrameInfo->stAddress.ui32TiledBase, pstFrameInfo->stAddress.ui32Y,
							pstFrameInfo->stAddress.ui32Cb, pstFrameInfo->stAddress.ui32Cr,
	...
	logm_debug(vdec_vdu, "%2d|%3d|%d|%d|%d|%d\n",

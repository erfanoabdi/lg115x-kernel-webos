...
static int vdc_support_hevc=0;
module_param(vdc_support_hevc, int, 0644);
...
static BOOLEAN _VDC_WorkNotiType(VDC_HANDLE_T hVdcCh, SINT8 si8FrameIndex, VDU_NOTI_T eNotiType);
...
static void _VDC_DecPostProc(VDC_HANDLE_T hVdcCh,VDU_RET_T eRetVal);
...
void VDC_Init(void)
{
    ...
    for(hVdcCh=0; hVdcCh<VDEC_NUM_OF_CHANNEL; hVdcCh++)
    {
        ...
        spin_lock_init(&gstVdcDb[hVdcCh].callback_lock);
    ...
    gstVdcCoreInfo.ui32NumofBusyCh = 0;
    atomic_set(&gstVdcCoreInfo.atFeedHandle, 0);
...
VDC_HANDLE_T VDC_Open(VDC_OPEN_PARAM_T* pstOpenParam)
{
    ...
    log_noti("opened VDCch:%d VDUch:%d VESch:%d clear_q:%p decoded_q:%p codec:%s usage:%d linear_buf_mode:%d psearch:%d thumb:%d \n"
            ...
            ,stOpenParam.ui16ValidThreshold,stOpenParam.bOneFrameDecoding);
...
VDC_RET_T VDC_Close(VDC_HANDLE_T hVdcCh)
{
    ...
    log_noti("ch:%d close start, feed_handle:%d, decoding:%d callback_processing:%d \n"
            , hVdcCh, atomic_read(&gstVdcCoreInfo.atFeedHandle), eStatus, atomic_read(&gstVdcDb[hVdcCh].atIsDecoding) );
            ...
            && VDU_Flush(gstVdcDb[hVdcCh].hVduCh) == VDU_RET_OK )
    {
        ...
        for(idx = 0;idx<gstVdcDb[hVdcCh].ui32DPBSize;idx++)
        {
            ...
            if(gstVdcDb[hVdcCh].ui32FrameFlags[idx] == 0)
                gstVdcDb[hVdcCh].ui32FrameFlags[idx] = -1;
        }
            ...
            ui32ReadPtr != VDU_INVALID_ADDRESS )
    {
        ...
        switch (gstVdcDb[hVdcCh].stCodecInfo.eCodecType)
        {
            ...
            case VDC_CODEC_MPEG2_HP:
                /* in case of mpeg2 pdec path, hw decoder consumes au size -2,
                 * so compensate it.*/
                if (gstVdcDb[hVdcCh].bLive || gstVdcDb[hVdcCh].bUsePdec)
                {
                    log_user1("ch:%d mpeg2 read ptr: 0x%X -> 0x%X \n"
                            ,hVdcCh,ui32ReadPtr,ui32ReadPtr+2);
                    ui32ReadPtr = CEILING_BYTES(ui32ReadPtr);
                }
                break;
        }
        ...
        switch(eNotiType)
        {
            ...
            case VDU_NOTI_CPB_EMPTY:
                ...
                if( gstVdcDb[hVdcCh].eLinearBufMode != VDC_LINEAR_MODE_DYNAMIC && si8DecodedIndex >= 0)
                    gstVdcDb[hVdcCh].ui32FrameFlags[si8DecodedIndex]++;
                break;
            case VDU_NOTI_FRAME_SKIP:
            case VDU_NOTI_SKIP_EMPTY:
                log_user1("ch:%d frame skipped rd:%X \n",hVdcCh, ui32ReadPtr);
        ...
        if( gstVdcDb[hVdcCh].ui32LastFeededPtr > ui32ReadPtr &&
                gstVdcDb[hVdcCh].ui32LastFeededPtr - ui32ReadPtr > VDC_READPOINTER_MISMATCH_THRESHOLD )
...
static void _VDC_MakeDispInfo(VDC_HANDLE_T hVdcCh, decoded_buffer_t *stDecBuf, VDU_FRM_INFO_T *pstFrameInfo)
{
    ...
    switch (pstFrameInfo->stDispInfo.eFrameMapType)
    {
        default: // FALL-THROUGH
        case VDU_FRM_MAP_TILED_MIXED_V:
            stDecBuf->vdo_map_type = DISPQ_MIXED_MAP_DPB; break;
        case VDU_FRM_MAP_TILED_FRAME_V:
            stDecBuf->vdo_map_type = DISPQ_FRAME_MAP_DPB; break;
        case VDU_FRM_MAP_LINEAR:
            stDecBuf->vdo_map_type = DISPQ_LINEAR_MAP_DPB; break;
        case VDU_FRM_MAP_TILED_HEVC:
            stDecBuf->vdo_map_type = DISPQ_HEVC_MAP_DPB; break;
        case VDU_FRM_MAP_TILED_HEVC10:
            stDecBuf->vdo_map_type = DISPQ_HEVC10_MAP_DPB; break;
        case VDU_FRM_MAP_TILED_8X4:
            stDecBuf->vdo_map_type = DISPQ_G1_MAP; break;
        case VDU_FRM_MAP_TILED_4X4:
            stDecBuf->vdo_map_type = DISPQ_G2_MAP; break;
    }
    ...
    stDecBuf->addr_tile_base = pstFrameInfo->stAddress.ui32TiledBase;
...
static void _VDC_MakeDebugInfo(VDC_HANDLE_T hVdcCh, CHAR* *pic_type, CHAR* *interlace, decoded_buffer_t *stDecBuf, UINT32 ui32Consumed, SINT64 ts)
{
    ...
    gstVdcDb[hVdcCh].stDebugInfo.ui32MapType = stDecBuf->vdo_map_type;
...
{
    if(gstVdcDb[hVdcCh].bLive || gstVdcDb[hVdcCh].bUsePdec){
        log_info("ch:%d PUSH!![%s][%s][%dx%d@%2d.%02d%s(%d:%d)]disp:%2d auib:%03d cpb:%06X rd:%06X dpb:%02d/%d LR:%d err:%2d fpa:%d fie:%d mt:%d dt:%5dus pts:%9u(0x%9u) \n"
                ,hVdcCh,pic_type,gstVdcDb[hVdcCh].stCodecInfo.sName,stDecBuf->width,stDecBuf->height
                ...
                ,gstVdcDb[hVdcCh].ui32DPBSize,stDecBuf->multi_picture,stDecBuf->error_blocks,stDecBuf->frame_packing_arrangement
                ,gstVdcDb[hVdcCh].stDebugInfo.bFieldCh,stDecBuf->vdo_map_type
                ,GET_DIFF(gstVdcDb[hVdcCh].ui32DecodingTime,TOP_HAL_GetGSTCC())*1000/90,stDecBuf->pts & 0x7FFFFFFF ,stDecBuf->pts);
    } else {
        log_info("ch:%d PUSH!![%s][%s][%dx%d@%2d.%02d%s(%d:%d)]disp:%2d auib:%03d cpb:%06X rd:%06X dpb:%02d/%d LR:%d err:%2d fpa:%d fie:%d mt:%d dt:%5dus ts:%6lldms(0x%9lld) \n"
                ,hVdcCh,pic_type,gstVdcDb[hVdcCh].stCodecInfo.sName,stDecBuf->width,stDecBuf->height
                ,gstVdcDb[hVdcCh].ui32FrameRate/100,gstVdcDb[hVdcCh].ui32FrameRate%100
                ,interlace,stDecBuf->interlace,stDecBuf->display_period&0x3
                ,stDecBuf->buffer_index,VES_AUIB_NumActive(gstVdcDb[hVdcCh].ui32VesCh)
...
static BOOLEAN _VDC_WorkNotiType(VDC_HANDLE_T hVdcCh, SINT8 si8FrameIndex, VDU_NOTI_T eNotiType)
{
    ...
    switch( eNotiType )
    {
        ...
        case VDU_NOTI_DPB_EMPTY:
            ...
            return TRUE;
        case VDU_NOTI_END_OF_STREAM:
            log_trace("ch:%d eos noti received \n",hVdcCh);
            gstVdcDb[hVdcCh].bRemainedData = FALSE;
            return TRUE;
        default:
...
static void _VDC_DecCallback(VDU_HANDLE_T hInstance, VDU_RESULT_T* pstResult)
{
    ...
    else if(gstVdcDb[hVdcCh].state == VDC_CLOSING)
    {
...
#endif
    spin_lock_irqsave(&gstVdcDb[hVdcCh].callback_lock, flag);
        ...
        if( gstVdcDb[hVdcCh].hVduCh != VDU_INVALID_HANDLE )
        {
            ...
            if( (pstFrameInfo->stDispInfo.bNonPairedField &&
                ...
                if( gstVdcDb[hVdcCh].eLinearBufMode != VDC_LINEAR_MODE_DYNAMIC )
                    gstVdcDb[hVdcCh].ui32FrameFlags[si8FrameIndex] = -1;
...
skip_displaying:
    spin_unlock_irqrestore(&gstVdcDb[hVdcCh].callback_lock, flag);
#ifdef CODE_FOR_CHECK_LOCK_TIME
    ...
    bFeedContinue = _VDC_WorkNotiType(hVdcCh, si8FrameIndex, pstResult->eNotiType);
    if( bFeedContinue ) {
        // channel state to idle
        atomic_set(&gstVdcDb[hVdcCh].atIsDecoding, 0);
        if(!gstVdcDb[hVdcCh].bLive) wake_up_all(&gstVdcDb[hVdcCh].wait_state_change);
        gstVdcCoreInfo.ui32NumofBusyCh--;
        if(gstVdcCoreInfo.ui32NumofBusyCh==0) STC_TIMER_Stop();
...
static void _VDC_Scheduler(void)
{
    ...
    atomic_set(&gstVdcCoreInfo.atFeedHandle, 1);

#ifdef CODE_FOR_CHECK_LOCK_TIME
    log_user2("sellect channel lock time:%dus \n",GET_DIFF(ui32LockTime,TOP_HAL_GetGSTCC())*1000/90);
#endif
    spin_unlock_irqrestore(&_VDC_Spinlock, flags);
...
static VDC_HANDLE_T _VDC_SellectChannel(void)
{
    ...
    if( atomic_read(&gstVdcCoreInfo.atFeedHandle) > 0 ){
        log_trace("no feed for duplicated handle \n");
        return hVdcCh;
    }
    ...
    for(hTempCh=0; hTempCh<VDEC_NUM_OF_CHANNEL; hTempCh++)
    {
        ...
        if(gstVdcDb[hTempCh].ui32VesCh != 0xFF &&
                _VDC_FeedPossible(hTempCh) == FALSE )
        {
            // only retrying at DPB Full, USRDPB_SET
            if( ( gstVdcDb[hTempCh].bWaitingClear == TRUE &&
                        gstVdcDb[hTempCh].bFlushed == FALSE ) ||
                    gstVdcDb[hTempCh].eWaitingBuffer == VDC_USRDPB_STATE_SET ){
                log_trace("ch:%d no need to feed, decoder needs direct decoding \n",hTempCh);
            } else {
                if(gstVdcDb[hTempCh].bRemainedData == FALSE){
                    log_trace ("ch:%d can't feed.. ves:%d auib %d, Used CPB %d\n",
                            hTempCh, gstVdcDb[hTempCh].ui32VesCh,
                            VES_AUIB_NumActive(gstVdcDb[hTempCh].ui32VesCh),
                            VES_CPB_GetUsedBuffer(gstVdcDb[hTempCh].ui32VesCh));
                    // feed timer reset at starving condition
                    gstVdcDb[hTempCh].ui32FeedStartTime = TOP_HAL_GetGSTCC();
                    continue;
                } else {
                    log_trace("ch:%d no auib but es remained at VDU \n",hTempCh);
#ifdef USE_VDC_RETRY
                    gstVdcDb[hVdcCh].bForceDecoding = TRUE;
#else
                    continue;
#endif
                }
...
#endif
    if(_VDC_PrepareFeed(hVdcCh,&stBodaAU)){
        goto feed;
    }
    else {
        goto finish;
...
decode:
    ...
    spin_lock_irqsave(&_VDC_Spinlock, flags);
    _VDC_DecPostProc(hVdcCh,eRetVal);
finish:
    spin_unlock_irqrestore(&_VDC_Spinlock, flags);
    return;
}
...
static void _VDC_InitInnerDatas(VDC_HANDLE_T hVdcCh)
{
    ...
    for(ui32idx=0;ui32idx<VDC_MAX_DPB_SIZE;ui32idx++)
    {
        ...
        gstVdcDb[hVdcCh].ui32FrameFlags[ui32idx] = -1;
...
static BOOLEAN _VDC_CheckUsableDPB(VDC_HANDLE_T hVdcCh)
{
    ...
    while( DISP_CLEAR_Q_Pop( gstVdcDb[hVdcCh].clear_q, &stClearQ) )
    {
        if(gstVdcDb[hVdcCh].hVduCh != VDU_INVALID_HANDLE &&
            ...
            gstVdcDb[hVdcCh].ui32FrameFlags[stClearQ.ui32FrameIdx] = -1;
    ...
    for(idx = 0;idx<gstVdcDb[hVdcCh].ui32DPBSize;idx++)
    {
        ...
        else if(gstVdcDb[hVdcCh].ui32FrameFlags[idx] == 0)
        {
            gstVdcDb[hVdcCh].stDebugInfo.sDpbState[idx] = '*';
            gstVdcDb[hVdcCh].ui32NumofDPBOccupied++;
        }
        else if(gstVdcDb[hVdcCh].ui32FrameFlags[idx] == -1)
        {
            gstVdcDb[hVdcCh].stDebugInfo.sDpbState[idx] = '-';
            bSpace = TRUE;
        }
...
auib_pop:
            ...
            VES_AUIB_Pop(gstVdcDb[hVdcCh].ui32VesCh, &stAUInfo))
    {
        ...
        //log_user3("ch:%d Feed AU PTS:%9u DTS:%9u PTSorDTS:%d \n", hVdcCh,
...
save:
    ...
    if( unlikely ( ts == gstVdcDb[hVdcCh].si64prevSavedTS &&
                ...
                si8FrameIndex < 0))
    {
        log_user1("ch:%d discard same pts %llu \n",hVdcCh,ts);
        ts = VDC_UNKNOWN_PTS;
    }
...
static void _VDC_DefineTsOrder(VDC_HANDLE_T hVdcCh)
{
    ...
    for(index=0;index<ui32NumofAu;index++)
    {
        if(VES_AUIB_GetAUInfo(gstVdcDb[hVdcCh].ui32VesCh, &stAUInfo))
        {
            ...
            log_user1("ch:%d index:%d ts:%lld \n",
                    hVdcCh, index, stAUInfo.ui64TimeStamp);
            /* break if timestamp order is pts */
            if( (prevTs > 0 && stAUInfo.ui64TimeStamp < prevTs
                 && stAUInfo.ui64TimeStamp != VDC_UNKNOWN_PTS )
            ...
            if (prevTs == stAUInfo.ui64TimeStamp
                    || stAUInfo.ui64TimeStamp == VDC_UNKNOWN_PTS)
            {
                log_user1("ch:%d index:%d prevts:%lld ts:%lld \n",
                        hVdcCh, index, prevTs,stAUInfo.ui64TimeStamp);
                ui32SamePtsCounter++;
            }
            else
            {
...
void VDC_Print_Stat(struct seq_file *file)
{
    ...
    for(hVdcCh=0; hVdcCh<VDEC_NUM_OF_CHANNEL; hVdcCh++)
    {
        ...
        VDC_DPRINT("FPA(3D) / LR(seq_3d) / map  : %d / %d / %d \n",gstVdcDb[hVdcCh].stDebugInfo.ui32FramePackingArrange,gstVdcDb[hVdcCh].stDebugInfo.ui32LRInfo,gstVdcDb[hVdcCh].stDebugInfo.ui32MapType);
...
void VDC_Print_Cmd(struct seq_file *file)
{
    ...
    VDC_DPRINT("  h <0:disable, 1:enable>   :enable/disable support hevc [default:disabled] \n");
    ...
    for(i=0;i<10 && i<ui32Param;i++)
    {
        ...
        switch (eCurState )
        {
            case VDC_DEBUG_CMD_IDLE:
                {
                    switch (*apParams[i])
                    {
                        ...
                        case 'h':
                        case 'H':
                            log_debug("support hevc \n");
                            eCurState = VDC_DEBUG_CMD_SUPPORT_HEVC;
                            break;
                        default:
                            break;
                    }
            ...
            case VDC_DEBUG_CMD_FORC_CODEC:
                ...
                break;
            case VDC_DEBUG_CMD_SUPPORT_HEVC:
                {
                    log_debug("support hevc? %d \n",i32Params[i]);
                    vdc_support_hevc = i32Params[i];
                }
                break;
            default:

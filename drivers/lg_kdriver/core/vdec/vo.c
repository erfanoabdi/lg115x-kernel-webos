...
struct vo_output_info output_info[4];
		...
		int *width, int *height)
{
	...
	if (framerate_num && framerate_den)
	{
		...
		*framerate_den = 0;
		...
		{
			*framerate_num = output_cmd (now->sync,
					output_cmd_get_framerate_num, 0);
			*framerate_den = output_cmd (now->sync,
					output_cmd_get_framerate_den, 0);
		}
...
#define seq_print_vo(v,f)	seq_printf (m, #v"="f"\n", vo->v)
	...
	seq_printf (m, "calling_write=%d\n", atomic_read (&vo->calling_write));
		...
		const char __user *buf, size_t len, loff_t *off)
{
	...
	while (len >= sizeof(struct vo_write))
	{
		...
		pic->ui32Tiled_FrameBaseAddr = vo->vdo_info->tile_base;
		pic->ui32DpbMapMode = vo->vdo_info->map_type;
		...
		VSync_HAL_IRQ_Enable ();
		spin_unlock_irqrestore (&output_info_lock, flag);
	}

	atomic_dec (&vo->calling_write);

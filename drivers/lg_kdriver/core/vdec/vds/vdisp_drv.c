...
#endif
static BOOLEAN is_uhd_seamless = FALSE;
		...
		VDISP_STATUS_CB fpStatusCb)
{
	...
	u8VSyncCh = VSync_Open(u8VdispCh, bIsDualDecoding, 30, 1, FALSE);

	DE_IF_SelVsync((DE_IF_DST_E)eOutPort, u8VSyncCh);
	//DE_IF_SelVsync((DE_IF_DST_E)eOutPort+2, u8VSyncCh);	// for raster mode

	vdisp_reset_handler(pstVdispCh);
	pstVdispCh->Config.eDeIfDstCh = (DE_IF_DST_E)u8VdispCh;//(DE_IF_DST_E)eOutPort;
...
void VDISP_SetDisplayOffset(UINT8 u8VdispCh, UINT32 u32DisplayOffset_ms)
{
	...
	PTS_Debug_Set_DisplayOffset(pstVdispCh->pstSync, u32DisplayOffset_ms);
...
*/
	...
	PTS_Debug_Set_DisplayOffset(gsVdisp[u8VdispCh].pstSync, DisplayOffset);
...
#if defined (CHIP_NAME_d14)
	if(is_uhd_seamless && pstVdispCh->Config.eDeIfDstCh==1 && delayoffsetflag == 0)
	{
		ui8FrameCnt++;
		if(ui8FrameCnt == 3)
		{
...
void VDISP_SeamlessModeNoti(UINT8 ui8SeamlessMode)
{
	...
	if (ui8SeamlessMode==0)
		is_uhd_seamless = FALSE;
	else
		is_uhd_seamless = TRUE;
...
void VDISP_DebugCmd(UINT32 u32nParam, CHAR *apParams[])
{
	...
	{
	case 0:
		VDISP_SetOutPort(u8VdispCh, au32Params[3]);
		break;

...
#define	SYNC_DCARD_THOLD_LIVE				(300)	// =~ 10sec
...
********************************************************************************
*/
void PTS_Debug_Set_DisplayOffset(SYNC_T *pstSync, UINT32 u32DispOffsetMs)
{
	if ( pstSync == NULL )
	{
		...
		UINT32 ui32PTS, UINT32 u32DispOffsetMs, BOOLEAN bSmoothing)
{
	...
	if ( pstSync->Config.bUseGstc )
	{
		...
		else
		{
			...
			if ( i32Speed > 0 )
			{
				i32PTS_Offset = (SINT32)PTS_GET_PTS_OFFSET(ui32BasePTS,
						ui32StreamTime);
				u32Speed = i32Speed;
			}
			else
			{
				i32PTS_Offset = -((SINT32)PTS_GET_PTS_OFFSET(ui32StreamTime,
							ui32BasePTS));
				u32Speed = -i32Speed;
			}
			...
			log_debug("[PTS%d][DBG:0x%X] STC Match - Speed: %d, BaseTime: 0x%X"
					", PTS:0x%X(0x%X), STC:0x%X, Occupancy: %d, "
					"FrameDuration:0x%X pbase %d poff %d str %d\n",
	...
	else
	{
		u32CompenPts = PTS_CHECK_PTS_WRAPAROUND(
				ui32StreamTime + ui32DisplayOffset_90kTick);

 ...
 *   Static Variables
 *----------------------------------------------------------------------------*/
static DE_IF_T		gstDeIf[DE_IF_DST_MAX];
...
void DE_IF_Init(void)
{
	...
	for (u8DeIfCh = 0; u8DeIfCh < DE_IF_NUM; u8DeIfCh++)
	{
...
{
	// TODO : DE_IF_NUM_OF_CHANNEL, DE_IF_NUM, DE_IF_DST_MAX ....clarify these
	if (u8DeIfCh >= DE_IF_NUM)
	{
		log_error("[DE_IF%d][Err] wrong channel! %s\n", u8DeIfCh, __FUNCTION__ );
		return 0xFF;
...
inline BOOLEAN DE_IF_UpdateDisplay(UINT8 u8DeIfCh, UINT32 ui32SyncField)
{
	...
	{
		if (pstDeIf->eTargetDe >= DE_IF_DST_MAX)
	...
	if (pstDeIf->repeat_cnt!=0)
	{
		...
		if (pstDeIf->repeat_cnt>0)
		{
			if (frm_info->ui32DisplayInfo==DISPQ_SCAN_PROG &&
				pstDeIf->repeat_cnt>1)
				pstDeIf->repeat_cnt-=2;
			else
				pstDeIf->repeat_cnt-=1;
...
void DE_IF_SeamlessModeNoti(UINT8 ui8bSeamlessMode)
{
	DIF_HAL_SetSeamless(ui8bSeamlessMode);
}
#endif

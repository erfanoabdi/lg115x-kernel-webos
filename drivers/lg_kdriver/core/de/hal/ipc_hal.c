...
int ipc_hal_init(void)
{
	...
	if (!LX_COMP_CHIP(lx_chip_rev(), LX_CHIP_H15) ) {
		g_ipc_client[MCU_ID_0].write = DE_KIPC_SetData;
		g_ipc_client[MCU_ID_0].read  = DE_KIPC_GetData;

		g_ipc_client[MCU_ID_1].write = DE_KIPC_SetData;
		g_ipc_client[MCU_ID_1].read  = DE_KIPC_GetData;
	} else
#endif

...
#define TX_RING_SIZE_DEFAULT	64
#define RX_RING_SIZE_DEFAULT	192
...
static int emac_mii_bus_reset(struct mii_bus *mii)
{
	...
	void __iomem *base = priv->reset_gpio_base;
	unsigned long ctrl = 1 << (priv->reset_gpio_num - 24);
...
static int emac_mdio_probe(struct mii_bus *mii)
{
	...
	priv->phy_dev = phy_connect(dev, dev_name(&priv->phy_dev->dev),
	                            emac_phy_notifier, priv->phy_mode);
...
static int emac_free_tx(struct net_device *dev)
{
	...
	for (i = 0; i < priv->txd_size; ++i) {
		...
		dma_unmap_single(&dev->dev, priv->txd_virt[i].addr1,
...
static int emac_free_rx(struct net_device *dev)
{
	...
	for (i = 0; i < priv->rxd_size; ++i) {
		...
		dma_unmap_single(&dev->dev, priv->rxd_virt[i].addr1,
...
static int emac_init(struct net_device *dev)
{
	...
	if(!(priv->desc_virt = ioremap(priv->desc_phys, priv->desc_size))) {
		netdev_err(dev, "failed to ioremap\n");
		err = -ENXIO;
		goto _quit_1;
	}
	...
	priv->rxd_virt = priv->desc_virt + sizeof(struct emac_txd) *
	                 priv->txd_size;
...
static int emac_probe(struct platform_device *pdev)
{
	...
	priv->desc_size = pdata->desc_size;

	if (!request_mem_region(priv->desc_phys, priv->desc_size, "emac-mem")) {
		dev_err(&pdev->dev, "failed to request_mem_region\n");
		err = -ENXIO;
		goto _quit_2;
	}
	...
	} else if (pdata && is_valid_ether_addr(pdata->mac_addr)) {
	...
	} else if (pdata && pdata->phy_mode > 0) {
		priv->phy_mode = pdata->phy_mode;
	} else {
		priv->phy_mode = PHY_INTERFACE_MODE_MII;
	}

	priv->txd_size = TX_RING_SIZE_DEFAULT;
	priv->rxd_size = RX_RING_SIZE_DEFAULT;
	...
	if (dev_node) {
		priv->reset_gpio_base = of_iomap(dev_node, 1);
		of_property_read_u32(dev_node, "reset-gpio-num",
					&priv->reset_gpio_num);
	}
	else {
		priv->reset_gpio_base = NULL;
		priv->reset_gpio_num = NULL;
	}
...
_quit_3:
	release_mem_region(priv->desc_phys, priv->desc_size);
_quit_2:
...
static int emac_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	struct emac_priv *priv = netdev_priv(netdev);
...
static int emac_resume(struct platform_device *pdev)
{
	...
	if (netif_running(netdev)) {
		...
		phy_attach(netdev, priv->mii_bus->id, priv->phy_mode);
		emac_open(netdev);

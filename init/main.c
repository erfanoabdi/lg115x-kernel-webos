	...
	size_t count, loff_t *ppos)
{
	...
	if (strlen(buffer) > 16 && copy_from_user(kbuf, buffer, count))
			return  -EFAULT;

	if (usercalls_stat == 0 && strlen(buffer) > 0) {
		do_user_initcalls(buffer);
...
static int __ref kernel_init(void *unused)
{
	...
	if (!run_init_process("/mnt/lg/lginit/lginit"))
		return 0;
#endif
